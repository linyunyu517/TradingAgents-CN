...
// 🐛 [BUG-024] 适应性轮询：running 标签页 5s→15s，降低无效请求频率
const setupPolling = () => {
  clearTimeout(timer)
  // 定期刷新列表（仅当 running 标签页活跃时）
...
const connectTaskWebSocket = (taskId: string) => {
  ...
  try {
    ...
    // [BUG-4 FIX] 修正 WebSocket URL 路径 + 添加 token 认证参数
    const wsUrl = `${wsProtocol}//${host}/api/analysis/ws/task/${taskId}?token=${encodeURIComponent(_token)}`

    const ws = new WebSocket(wsUrl)

    ws.onopen = () => {
      console.info(`✅ WebSocket 连接成功: ${taskId}`)
    }
      ...
      try {
        ...
        if (message.type === 'progress_update') {
          ...
          if (taskIndex >= 0) {
            ...
            console.info(`📊 更新任务进度: ${taskId} -> ${message.progress}%`)
          }
        }
      } catch (e) {
    ...
    ws.onclose = () => {
      console.info(`🔌 WebSocket 断开: ${taskId}`)
