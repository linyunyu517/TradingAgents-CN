...
const handleRunFetchModels = async () => {
  ...
  try {
    ...
    console.info('🔍 [fetch-models] provider=', fetchingProvider.value)
    console.info('🔍 [fetch-models] payload=', requestPayload)
    const response = await configApi.fetchProviderModels(fetchingProvider.value.id, requestPayload)
    // console.log('📦 [fetch-models] response=', response)  // 保留用于调试
