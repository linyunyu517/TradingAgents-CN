...
const loadModelCatalog = async () => {
  try {
    ...
    console.info('✅ 模型目录加载成功:', Object.keys(catalogMap))
  } catch (error) {
...
const handleProviderChange = async (provider: string) => {
  ...
  if (modelOptions.value.length === 0) {
    console.info(`⚠️ 供应商 ${provider} 没有模型数据，重新加载模型目录...`)
...
const handleModelSelect = (modelCode: string) => {
  ...
  if (selectedModel) {
    ...
    if (modelInfo) {
      console.info('📋 自动填充模型信息:', modelInfo)
...
watch(
  ...
  (config) => {
    if (config) {
      ...
      console.info('📝 编辑模式加载配置:', formData.value)
    } else {
  ...
  async (visible) => {
    if (visible) {
      ...
      if (props.config) {
        ...
        console.info('📝 对话框打开，加载配置:', formData.value)
      } else {
        // 新增模式：使用默认值
        formData.value = { ...defaultFormData }
        // 如果有供应商，加载其模型列表
...
const handleSubmit = async () => {
  ...
  try {
    ...
    console.info('🚀 提交大模型配置:', submitData)
...
const loadProviders = async (showSuccessMessage = false) => {
  ...
  try {
    ...
    console.info('✅ 加载厂家列表成功:', availableProviders.value.length)
