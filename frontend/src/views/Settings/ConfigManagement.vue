...
import { Setting, Cpu, DataBoard, Coin, Tools, Download, Upload, Plus, Refresh, Key, OfficeBuilding, CircleCheck, Collection } from '@element-plus/icons-vue'
...
const availableModelsForProvider = (providerId: string) => {
  // console.log('🔍 获取厂家模型:', providerId)  // 保留用于调试
  // console.log('📊 所有大模型配置:', llmConfigs.value)  // 保留用于调试
  if (!providerId) {
    // console.log('⚠️ 厂家ID为空')  // 保留用于调试
    return []
  }
  const models = llmConfigs.value.filter(config => {
    // console.log(`检查模型: ${config.model_name}, provider: ${config.provider}, enabled: ${config.enabled}`)  // 保留用于调试
    return config.provider === providerId && config.enabled
  })
  // console.log(`✅ 找到 ${models.length} 个可用模型:`, models)  // 保留用于调试
...
const loadProviders = async () => {
  ...
  try {
    console.info('🔄 开始加载厂家列表...')
    const providerList = await configApi.getLLMProviders()
    // console.log('📊 厂家列表响应:', providerList)  // 保留用于调试
    providers.value = sortProvidersByNewest(providerList)
    console.info('✅ 厂家列表加载成功，数量:', providerList.length)
  } catch (error) {
...
// 刷新大模型配置数据
const _refreshLLMConfigs = () => {
  buildLLMConfigGroups()
}

// 获取厂家标签类型
const getProviderTagType = (provider: string) : any => {
...
// 🆕 获取能力等级标签类型
const getCapabilityLevelType = (level: number) : any => {
...
const addModelToProvider = (providerRow: any) => {
  ...
  currentLLMConfig.value = {
    ...
    model_name: '',
      // @ts-expect-error display_name exists at runtime
    display_name: '',
    description: '',
    enabled: true,
    max_tokens: 4000,
...
const testProviderAPI = async (provider: LLMProvider) => {
  try {
    console.info('🔍 测试厂家API:', provider)
    // console.log('📋 厂家ID:', provider.id)  // 保留用于调试
    // console.log('📋 厂家名称:', provider.display_name)  // 保留用于调试
...
// 设置默认LLM
const _setDefaultLLM = async (modelName: string) => {
  try {
...
const testLLMConfig = async (config: LLMConfig) => {
  try {
    console.info('🧪 测试LLM配置:', config)
    // console.log('📋 厂家:', config.provider)  // 保留用于调试
    // console.log('📋 模型名称:', config.model_name)  // 保留用于调试
    // console.log('📋 显示名称:', config.model_display_name)  // 保留用于调试
    // console.log('📋 API基础URL:', config.api_base)  // 保留用于调试
...
const _setDefaultDataSource = async (name: string) => {
  try {
...
const testDatabase = async (config: DatabaseConfig) => {
  try {
    console.info('🧪 测试数据库配置:', config)
    // console.log('📋 配置名称:', config.name)  // 保留用于调试
    // console.log('📋 配置类型:', config.type)  // 保留用于调试
    // console.log('📋 主机地址:', config.host)  // 保留用于调试
    // console.log('📋 端口:', config.port)  // 保留用于调试
