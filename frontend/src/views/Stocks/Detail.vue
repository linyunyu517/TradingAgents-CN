      ...
      <el-col :span="18">
        ...
        <el-card v-if="analysisStatus==='running' || lastAnalysis" shadow="hover" class="analysis-detail-card" id="analysis-detail">
          ...
          <div v-else class="detail">
            ...
            <!-- v1.0.1 HPC 融合分析模块状态 -->
            <div v-if="lastAnalysis?.modules_enabled || lastAnalysis?.fusion_mode !== undefined || lastAnalysis?.performance_summary" class="hpc-section">
              <el-divider />
              <div class="hpc-section-title">⚙️ HPC 融合分析引擎</div>
              <div class="hpc-content">
                <div v-if="lastAnalysis?.modules_enabled" class="modules-status-bar">
                  <el-tag
                    v-for="(enabled, mod) in lastAnalysis.modules_enabled"
                    :key="String(mod)"
                ...
                <div v-if="lastAnalysis?.fusion_mode !== undefined" class="fusion-mode-indicator">
                  <el-tag :type="lastAnalysis.fusion_mode ? 'warning' : 'info'" effect="dark" size="small">
                    🔀 融合模式: {{ lastAnalysis.fusion_mode ? '已启用' : '未启用' }}
                  </el-tag>
                </div>
                <div v-if="lastAnalysis?.performance_summary" class="performance-summary">
                  <el-descriptions :column="3" size="small" border>
                    <el-descriptions-item
                      v-for="(value, key) in lastAnalysis.performance_summary"
                      :key="String(key)"
...
import { TrendCharts, Star, Refresh, Link, Document, Clock, Reading, CreditCard, Delete, Check, Close } from '@element-plus/icons-vue'
...
const syncForm = reactive({
  ...
  dataSource: 'akshare' as 'akshare' | 'baostock',
  days: 365
})
...
function formatReportName(key: string): string {
  ...
  const nameMap: Record<string, string> = {
    ...
    // HPC 子模块报告 (v1.0.1 新增)
    'l_iwm_report': '🧠 动量预测 (L-IWM)',
    'hsrc_mc_report': '🎲 蒙特卡洛模拟 (HSRC-MC)',
    'aif_report': '🤖 AI 融合报告 (AIF)',
