                    ...
                    <div class="overall-progress-info">
                      <div class="progress-stats">
                        ...
                        <div class="stat-item">
                          ...
                          <div class="stat-value">{{ formatTime(progressInfo.remainingTime, progressInfo.elapsedTime) }}</div>
                        </div>
                        <div class="stat-item">
          ...
          <el-col :span="24">
            <el-card class="results-card" shadow="hover">
              ...
              <div class="results-content">
                    ...
                    <div class="decision-main">
                      ...
                      <div class="decision-metrics">
                        ...
                        <div class="metric-item">
                          ...
                          <span class="value">{{ safePct(analysisResults.decision.confidence) }}%</span>
                        ...
                        <div class="metric-item">
                          ...
                          <span class="value">{{ safePct(analysisResults.decision.risk_score) }}%</span>
                ...
                <!-- v1.0.1 HPC 融合分析模块状态 -->
                <div v-if="analysisResults.modules_enabled || analysisResults.fusion_mode !== undefined || analysisResults.performance_summary" class="hpc-section">
                  <h4>⚙️ HPC 融合分析引擎</h4>
                  <div class="hpc-content">
                    <!-- 模块启用状态 -->
                    <div v-if="analysisResults.modules_enabled" class="modules-status-bar">
                      <el-tag
                        v-for="(enabled, mod) in analysisResults.modules_enabled"
                        :key="String(mod)"
                    ...
                    <!-- 融合模式 -->
                    <div v-if="analysisResults.fusion_mode !== undefined" class="fusion-mode-indicator">
                      <el-tag :type="analysisResults.fusion_mode ? 'warning' : 'info'" effect="dark" size="small">
                        🔀 融合模式: {{ analysisResults.fusion_mode ? '已启用' : '未启用' }}
                      </el-tag>
                    </div>

                    <!-- 性能摘要 -->
                    <div v-if="analysisResults.performance_summary" class="performance-summary">
                      <el-descriptions :column="3" size="small" border>
                        <el-descriptions-item
                          v-for="(value, key) in analysisResults.performance_summary"
                          :key="String(key)"
...
const submitAnalysis = async () => {
  ...
  try {
    ...
    console.info('🔍 分析响应数据:', response)
    console.info('🔍 响应数据结构:', response.data)
    console.info('🔍 任务ID:', response.data?.task_id)
    ...
    console.info('✅ 任务ID设置成功:', currentTaskId.value)
    ...
    setTimeout(async () => {
      try {
        ...
        console.info('🔄 立即查询状态:', status)
        console.info('🔄 当前 analysisStatus:', analysisStatus.value)
        if (status.status === 'running' || status.status === 'processing') {
          analysisStatus.value = (status.status === 'running' || status.status === 'processing') ? 'running' : status.status
...
// 🐛 [BUG-024] 适应性轮询：根据经过时间动态调整轮询间隔
// 减少 pending 状态下的无效请求，降低后端负载
let _pollingPollCount = 0
let _pollingStartTime = 0  // 使用 let 以支持重置
...
const startPollingTaskStatus = () => {
  ...
  console.info('🔄 开始轮询任务状态:', currentTaskId.value)
  _pollingPollCount = 0
  _pollingStartTime = Date.now()

  // 使用 setTimeout 递归实现动态间隔轮询
  const scheduleNextPoll = (delayMs: number) => {
    ...
    pollingTimer.value = setTimeout(async () => {
      ...
      _pollingPollCount++
      const elapsed = (Date.now() - _pollingStartTime) / 1000
      ...
      let nextDelay = 5000
      if (elapsed < 30) {
        nextDelay = 2000     // 前 30 秒：2s 快速检测
      } else if (elapsed < 120) {
        nextDelay = 5000     // 30-120 秒：5s 正常轮询
      } else {
        nextDelay = 10000    // 超过 2 分钟：10s 降频轮询
      }
  ...
  scheduleNextPoll(2000)  // 首次轮询：2s 后开始
}
...
const updateProgressInfo = (status: any) => {
  ...
  // 🐛 [BUG-030] 增加多重 fallback：current_step_name → progress_msg → 当前值 → 默认文案
  ...
  } else if (status.progress_msg) {
    // console.log('📋 降级使用 progress_msg:', status.progress_msg)
    progressInfo.value.currentStep = status.progress_msg
  } else if (!progressInfo.value.currentStep) {
    // console.log('📋 无步骤信息，使用默认文案')
...
const getAnalysisReports = (data: any) => {
  ...
  const reportMappings = [
    ...
    // HPC 子模块报告 (v1.0.1 新增)
    { key: 'l_iwm_report', title: '🧠 动量预测 (L-IWM)', category: 'HPC 融合分析' },
    { key: 'hsrc_mc_report', title: '🎲 蒙特卡洛模拟 (HSRC-MC)', category: 'HPC 融合分析' },
    { key: 'aif_report', title: '🤖 AI 融合报告 (AIF)', category: 'HPC 融合分析' },
    { key: 'diffusion_report', title: '🔄 扩散模型预测', category: 'HPC 融合分析' },
    { key: 'fusion_report', title: '🔗 融合决策报告', category: 'HPC 融合分析' },
...
/**
 * 安全百分比格式化：处理 null / undefined / NaN
 * - 如果值有效（0-1 的有限数字），返回 "85.0" 格式的百分制字符串
 * - 如果值无效（null/undefined/NaN），返回 "-"
 */
...
const formatTime = (seconds: number, elapsed?: number) => {
  if (!seconds || seconds <= 0) {
    // remaining_time <= 0 时显示已运行时间，避免"进度冻结"感知
    if (elapsed && elapsed > 0) {
      return `已运行 ${formatTime(elapsed)}`
    }
    return '超过预估时间，等待中...'
  }
...
const updateAnalysisSteps = (status: any) => {
  ...
  const _statusSummary = analysisSteps.value.map((s, i) => `${i}:${s.status}`).join(', ')
...
onMounted(async () => {
  ...
  // 优先从 authStore.user.preferences 读取，其次从 _appStore.preferences 读取
  ...
  if (userPrefs) {
    ...
    // 降级到 _appStore.preferences
    if (_appStore.preferences.defaultMarket) {
      analysisForm.market = _appStore.preferences.defaultMarket as MarketType
    }
    if (_appStore.preferences.defaultDepth) {
      analysisForm.researchDepth = parseInt(_appStore.preferences.defaultDepth)
    }
