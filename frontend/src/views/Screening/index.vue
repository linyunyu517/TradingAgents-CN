...
const performScreening = async () => {
  ...
  try {
    ...
    const res = await screeningApi.run(payload as any, { timeout: 120000 })
...
const loadFieldConfig = async () => {
  ...
  try {
    ...
    console.info('字段配置加载成功:', fieldConfig.value)
  } catch (error) {
...
const loadIndustries = async () => {
  try {
    ...
    console.info('行业列表加载成功:', industryOptions.value.length, '个行业')
  } catch (error) {
