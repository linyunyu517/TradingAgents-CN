...
export const useNotificationStore = defineStore('notifications', () => {
  ...
  function connectWebSocket() {
    try {
      ...
      socket.onclose = (event) => {
        console.info('[WS] 连接关闭:', event.code, event.reason)
        ...
        if (wsReconnectAttempts < maxReconnectAttempts) {
          ...
          console.info(`[WS] ${delay}ms 后重连 (尝试 ${wsReconnectAttempts + 1}/${maxReconnectAttempts})`)
  ...
  function handleWebSocketMessage(message: any) {
    switch (message.type) {
      case 'connected':
        console.info('[WS] 连接确认:', message.data)
        break
  ...
  // 🔥 连接 WebSocket
  function connect() {
    console.info('[Notifications] 开始连接...')
    connectWebSocket()
  }

  // 🔥 断开 WebSocket
  function disconnect() {
    console.info('[Notifications] 断开连接...')
    disconnectWebSocket()
  }
