        ...
        <div v-else-if="historyList.length > 0" class="history-list">
          <el-timeline>
            <el-timeline-item
              ...
              :type="getTimelineType(item.status) as any"
...
const fetchHistory = async (page = 1) => {
  try {
    ...
    if (response.success) {
      ...
      console.info(`📊 获取到 ${records.length} 条历史记录，总数: ${total}`)
...
// 获取时间线类型
const getTimelineType = (status: string): any => {
  const typeMap: Record<string, string> = {
    success: 'success',
    success_with_errors: 'warning',
    failed: 'danger',
