      ...
      <div class="control-content">
        ...
        <div class="sync-status-section">
          ...
          <div class="status-display">
            <el-tag 
              :type="getStatusType(syncStatus?.status) as any"
              size="large"
              class="status-tag"
            >
...
const startSync = async () => {
  try {
    ...
    if (response.success) {
      ...
      console.info('🚀 同步任务启动成功，当前状态:', responseStatus)
...
const startStatusPolling = () => {
  ...
  if (props.autoRefresh) {
    ...
    console.info('🔄 开始状态轮询，初始状态:', previousStatus)
    ...
    refreshTimer.value = setInterval(async () => {
      ...
      console.info(`🔍 轮询 #${pollCount}: ${previousStatus} -> ${currentStatus}`)

      // 检查状态变化，提供用户反馈
      if (previousStatus === 'running' && currentStatus && currentStatus !== 'running') {
        // 同步完成，显示结果通知
        console.info('🎉 检测到同步完成，状态从', previousStatus, '变为', currentStatus)
      ...
      if (currentStatus && !['running'].includes(currentStatus)) {
        console.info('🛑 检测到非运行状态，停止轮询:', currentStatus)
        stopStatusPolling()
        return
      }
...
const showSyncCompletionNotification = (status: string) => {
  console.info('📢 显示同步完成通知，状态:', status)
  ...
  console.info('📊 同步统计:', { total, inserted, updated, errors })
