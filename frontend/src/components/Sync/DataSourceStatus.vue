...
const fetchDataSourcesStatus = async () => {
  try {
    console.info('🔍 [DataSourceStatus] 开始获取数据源状态')
    loading.value = true
    error.value = ''
    ...
    console.info('🔍 [DataSourceStatus] API连接测试结果:', connectionOk)
    ...
    console.info('🔍 [DataSourceStatus] API响应:', response)

    if (response.success) {
      console.info('🔍 [DataSourceStatus] API调用成功，数据源数量:', response.data?.length || 0)
      console.info('🔍 [DataSourceStatus] 数据源详情:', response.data)
    ...
    loading.value = false
    console.info('🔍 [DataSourceStatus] 获取数据源状态完成')
  }
}
