...
const initApp = async () => {
  try {
    ...
    if (error.code === 'ECONNABORTED' || error.message?.includes('timeout')) {
      console.info('📱 离线模式：应用将在没有后端连接的情况下启动')
    }
  } finally {
    // 无论认证状态如何，都挂载应用
    app.mount('#app')
...
if (import.meta.env.DEV) {
  console.debug('🚀 TradingAgents-CN v1.0.0-preview 前端应用已启动')
  console.debug('📊 当前环境:', import.meta.env.MODE)
  console.debug('🔗 API地址:', import.meta.env.VITE_API_BASE_URL || '/api')
}
