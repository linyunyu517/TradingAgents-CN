...
export interface AnalysisResult {
  ...
  // v1.0.1 新增 HPC 融合分析字段
  modules_enabled?: import('./request').ApiResponse<{
    hpc_loop: boolean
    l_iwm: boolean
    hsrc_mc: boolean
    aif_engine: boolean
  ...
  hpc_reports?: Record<string, string>
}
...
export const analysisApi = {
  ...
  // 停止分析（已废弃：请使用 markTaskAsFailed）
  /** @deprecated 使用 markTaskAsFailed 替代 */
  ...
  getTaskResult(taskId: string): Promise<AnalysisResult>{
  ...
  // 分享分析结果（后端暂无此接口，保留为主持扩展预留）
  /** @deprecated 后端暂无分享接口 */
  shareAnalysis(_analysisId: string, _options: {
    expires_in?: number // 过期时间（秒）
    password?: string   // 访问密码
    public?: boolean    // 是否公开
  }): Promise<{ share_url: string; share_code: string }> {
    return Promise.reject(new Error("分享功能尚未实现"))
  },

  // 获取股票基础信息（后端暂无此接口，改为搜索兜底）
  /** @deprecated 使用 searchStocks 替代 */
  getStockInfo(symbol: string, market: string): Promise<{
    ...
    return request.get('/api/analysis/search', {
      ...
      throw new Error(`未找到股票 ${symbol} 的信息`)
    })
  },
...
/**
 * 🐛 Bug #20: WebSocket 客户端 — 支持心跳检测和自动重连
 */
export class AnalysisWebSocketClient {
  ...
  constructor(
    ...
    // 构建 WebSocket URL（支持相对路径自动补全）
    const proto = window.location.protocol === 'https:' ? 'wss:' : 'ws:'
    this.baseUrl = `${proto}//${window.location.host}`
    // [BUG-4 FIX] 从 localStorage 获取 token 用于认证
    this._authToken = (() => {
  ...
  connect(): void {
    ...
    const url = `${this.baseUrl}/api/analysis/ws/task/${this.taskId}?token=${encodeURIComponent(this._authToken)}`
    console.info(`🔌 [WS] 连接: ${url}`)
    this.ws = new WebSocket(url)

    this.ws.onopen = () => {
      console.info(`✅ [WS] 连接已建立: ${this.taskId}`)
    ...
    this.ws.onmessage = (event: MessageEvent) => {
      try {
        ...
        console.warn(`⚠️ [WS] 消息解析失败:`, e)
      }
    }

    this.ws.onclose = (event: CloseEvent) => {
      console.info(`🔌 [WS] 连接关闭 (code=${event.code}): ${this.taskId}`)
      this.stopHeartbeat()
      this.ws = null
      if (this.options.autoReconnect && !this.destroyed) {
        this.tryReconnect()
      }
    }
  ...
  private tryReconnect(): void {
    if (this.reconnectCount >= this.options.maxReconnects) {
      console.error(`❌ [WS] 已达最大重连次数 (${this.options.maxReconnects})`)
      return
    }
    this.reconnectCount++
    const delay = this.options.reconnectDelay * 1000
    console.info(`🔄 [WS] 第 ${this.reconnectCount} 次重连 (${delay}ms 后)...`)
