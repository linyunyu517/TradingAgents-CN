...
from app.services.progress.tracker import RedisProgressTracker, cleanup_stale_progress_files
...
def get_user_selections():
    ...
    console.print(
        create_question_box(
            "步骤 5: 研究深度 | Step 5: Research Depth", "选择您的研究深度级别 | Select your research depth level",
        ),
    )
    ...
    console.print(
        create_question_box(
            "步骤 6: LLM提供商 | Step 6: LLM Provider", "选择要使用的LLM服务 | Select which LLM service to use",
        ),
    )
...
def run_analysis():
    ...
    with Live(layout, refresh_per_second=DEFAULT_REFRESH_RATE):
        ...
        try:
            ...
            # 确定市场类型
            {"china_stock": "A股", "yahoo_finance": "港股" if ".HK" in selections["ticker"] else "美股"}
        ...
        # 方案C：初始化文件进度追踪
        task_id = f"cli_{selections['ticker']}_{selections['analysis_date']}_{int(time.time())}"
        progress_tracker = RedisProgressTracker(
            ...
            total_steps=len(selections["analysts"]) * 3 + 3,  # 分析师阶段 × 3 + 数据 + 交易 + 报告
        )
        ...
        for chunk in graph.graph.stream(init_agent_state, **args):
            ...
            # 方案C：更新进度文件（每轮chunk写入一次）
            try:
                completed_count = len(completed_analysts)
                total_phases = len(selections["analysts"]) + 4  # 分析师 + 数据 + 研究 + 交易 + 风控
                pct = min(int(completed_count / max(total_phases, 1) * 100), 95)
                progress_tracker.update_progress({
                    "status": "running",
                    "progress_percentage": pct,
                    "completed_analysts": list(completed_analysts),
                    "current_step": f"分析进度: {len(completed_analysts)}/{total_phases} 阶段完成",
                })
            except Exception:
                pass
...
def config():
    ...
    providers_table.add_row(
        "🌍 OpenAI", "gpt-4o, gpt-4o-mini, gpt-3.5-turbo", "✅ 支持 | Supported", "需要国外API | Requires overseas API",
    )
    providers_table.add_row(
        "🤖 Anthropic", "claude-3-opus, claude-3-sonnet", "✅ 支持 | Supported", "需要国外API | Requires overseas API",
    )
    providers_table.add_row(
        "🔍 Google AI", "gemini-pro, gemini-2.0-flash", "✅ 支持 | Supported", "需要国外API | Requires overseas API",
    )
...
def data_config(
    ...
    for name, path in directories.items():
        status = ("✅ 存在" if os.path.exists(path) else "❌ 不存在") if path and path != "未配置" else "⚠️ 未配置"
...
def examples():
    ...
    examples_table.add_row(
        "🇨🇳 阿里百炼", "examples/dashscope/demo_dashscope.py", "完整功能演示 | Full feature demonstration",
    )
    examples_table.add_row(
        "🇨🇳 阿里百炼", "examples/dashscope/demo_dashscope_simple.py", "简化测试版本 | Simplified test version",
    )
    examples_table.add_row("🌍 OpenAI", "examples/openai/demo_openai.py", "OpenAI模型演示 | OpenAI model demonstration")
    examples_table.add_row("🧪 测试", "tests/integration/test_dashscope_integration.py", "集成测试 | Integration test")
    examples_table.add_row(
        "📁 配置演示", "examples/data_dir_config_demo.py", "数据目录配置演示 | Data directory configuration demo",
    )
...
def test():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "tests/integration/test_dashscope_integration.py"], capture_output=True, text=True, cwd=".",
        )
