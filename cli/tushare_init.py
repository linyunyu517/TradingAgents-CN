...
async def check_database_status():
    ...
    try:
        ...
        if basic_count == 0:
            ...
            return False
        if extended_count / basic_count < 0.5:
...
async def run_full_initialization(
    historical_days: int, force: bool, multi_period: bool = False, sync_items: list | None = None,
):
    """运行完整初始化"""
    if sync_items:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Tushare数据初始化工具", formatter_class=argparse.RawDescriptionHelpFormatter,
    )
