...
def demo_stock_list_fetcher():
    """演示增强版股票列表获取功能"""
    logger.info("=== 增强版股票列表获取演示 ===")
    logger.info("\n功能特点:")
    logger.info("1. 从tdx_servers_config.json自动加载服务器配置")
    logger.info("2. 支持服务器故障转移")
    logger.info("3. 获取完整的股票、指数、ETF信息")
    logger.info("4. 自动数据清洗和去重")

    # 演示不同类型的数据获取
    data_types = {"stock": "股票", "index": "指数", "etf": "ETF基金", "all": "全部数据"}
    ...
    for type_key, type_name in data_types.items():
        ...
        try:
            ...
            logger.error(f"❌ 获取{type_name}数据时发生错误: {e!s}")

        # 只演示第一种类型，避免过多网络请求
        logger.warning("\n注意: 为避免过多网络请求，此演示只获取股票数据")
        logger.info("实际使用时可以获取所有类型的数据")
        break
...
def show_usage_examples():
    ...
    examples = [
        {"title": "获取所有股票数据", "code": """result = enhanced_fetch_stock_list(type_='stock')"""},
        {"title": "获取所有指数数据", "code": """result = enhanced_fetch_stock_list(type_='index')"""},
        {"title": "获取ETF数据", "code": """result = enhanced_fetch_stock_list(type_='etf')"""},
        {"title": "获取全部数据（股票+指数+ETF）", "code": """result = enhanced_fetch_stock_list(type_='all')"""},
        {
            "title": "启用服务器故障转移",
...
if __name__ == "__main__":
    ...
    # 演示功能（注释掉以避免实际网络请求）
    logger.info("\n")
    logger.info("如需测试实际功能，请取消下面代码的注释:")
    logger.info("# demo_stock_list_fetcher()")
