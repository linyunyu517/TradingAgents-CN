...
def verify_stock_data(db):
    ...
    # 2. 按市场统计
    print("\n🏢 市场分布:")
    market_pipeline = [{"$group": {"_id": "$sse", "count": {"$sum": 1}}}, {"$sort": {"count": -1}}]
    ...
    # 3. 按分类统计
    print("\n📊 分类分布:")
    category_pipeline = [{"$group": {"_id": "$sec", "count": {"$sum": 1}}}, {"$sort": {"count": -1}}]

    for category in collection.aggregate(category_pipeline):
        category_name = {"stock_cn": "股票", "etf_cn": "ETF基金", "index_cn": "指数", "bond_cn": "债券"}.get(
    ...
    # 查找平安相关股票
    ping_an_stocks = list(collection.find({"name": {"$regex": "平安", "$options": "i"}}).limit(5))
