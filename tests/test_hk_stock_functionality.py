    ...
    try:
        ...
        for ticker, expected in test_cases:
            ...
            if expected == "港股" and not market_info["is_hk"]:
                ...
                return False
            if expected == "中国A股" and not market_info["is_china"]:
                print(f"❌ {ticker} 应该被识别为中国A股")
                return False
            if expected == "美股" and not market_info["is_us"]:
...
def test_optimized_us_data_hk_support():
    ...
    try:
        ...
        data_text = get_us_stock_data_cached(
            symbol=hk_symbol, start_date=start_date, end_date=end_date, force_refresh=True,
        )
