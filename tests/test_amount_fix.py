...
async def test_amount_fix():
    ...
    df = await provider.get_historical_data(symbol=test_code, start_date=start_date, end_date=end_date, period="daily")
    ...
    doc = coll.find_one({"symbol": test_code, "period": "daily", "data_source": "tushare"}, sort=[("trade_date", -1)])
