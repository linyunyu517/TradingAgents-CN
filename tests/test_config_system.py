...
class TestStartupValidator:
    ...
    def test_config_item_creation(self):
        """测试配置项创建"""
        config = ConfigItem(
            key="TEST_KEY", level=ConfigLevel.REQUIRED, description="Test configuration", example="test_value",
        )
...
class TestConfigCompat:
    ...
    def test_track_usage(self):
        ...
        tracker = TokenTrackerCompat()
        tracker.track_usage(
            provider="test_provider", model_name="test_model", input_tokens=100, output_tokens=50, cost=0.01,
        )
