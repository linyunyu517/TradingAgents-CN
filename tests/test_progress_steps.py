...
async def start_analysis(token: str) -> str:
    ...
    async with (
        ...
        session.post(f"{BASE_URL}/api/analysis/single", headers=headers, json=analysis_data) as response,
    ):
        if response.status == 200:
            result = await response.json()
...
async def get_task_status(token: str, task_id: str) -> dict:
    ...
    async with (
        aiohttp.ClientSession() as session,
        session.get(f"{BASE_URL}/api/analysis/tasks/{task_id}/status", headers=headers) as response,
    ):
        if response.status == 200:
            result = await response.json()
            return result["data"]
...
def print_progress_info(status_data: dict, iteration: int):
    ...
    if steps:
        ...
        for i, step in enumerate(steps):
            status_icon = {"pending": "⏳", "current": "🔄", "completed": "✅", "failed": "❌"}.get(
                step.get("status", "pending"), "❓",
            )

            print(
                f"{i:<6} {status_icon} {step.get('status', 'N/A'):<10} {step.get('name', 'N/A'):<30} {step.get('weight', 0):.2%}",
            )
