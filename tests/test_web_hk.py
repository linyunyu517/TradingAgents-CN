    ...
    try:
        ...
        for code in valid_hk_codes:
            errors = validate_analysis_params(
                stock_symbol=code, analysis_date="2024-01-01", analysts=["market"], research_depth=3, market_type="港股",
            )
            if not errors:
                print(f"    ✅ {code} 验证通过")
            else:
...
def test_market_type_integration():
    ...
    try:
        ...
        market_configs = [
            ...
            {"market_type": "A股", "symbol": "000001", "currency": "¥", "expected_features": ["A股", "¥", "人民币"]},
            {"market_type": "美股", "symbol": "AAPL", "currency": "$", "expected_features": ["美股", "$", "美元"]},
        ]
