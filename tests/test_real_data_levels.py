...
def test_stock_with_all_levels(ticker, stock_name):
    ...
    # 测试所有级别
    test_levels = [(1, "级别1-快速"), (2, "级别2-标准"), (3, "级别3-详细"), (4, "级别4-深入"), (5, "级别5-全面")]
    ...
    for level_num, level_name in test_levels:
        ...
        try:
            ...
            result = toolkit.get_stock_fundamentals_unified.invoke(
                {"ticker": ticker, "start_date": start_date, "end_date": end_date, "curr_date": curr_date},
            )
            ...
            results[level_num] = {"level_name": level_name, "data": result, "analysis": analysis}
            ...
            traceback.print_exc()
            results[level_num] = {"level_name": level_name, "error": str(e)}
    ...
    if len(successful_results) >= 2:
        ...
        for level_num in sorted(successful_results.keys()):
            ...
            print(
                f"   {result['level_name']}: {length:,} 字符, {sections} 模块, {data_types} 数据类型, 日期范围: {date_range}",
            )
