...
def test_sse_task_connected_event(monkeypatch):
    ...
    monkeypatch.setattr(sse_router_mod, "get_redis_client", FakeRedis)
...
def test_sse_batch_connected_event(monkeypatch):
    ...
    monkeypatch.setattr(qsvc_mod, "get_redis_client", FakeRedis)
...
def test_worker_intervals_env_and_dynamic_override(monkeypatch):
    ...
    monkeypatch.setattr(analysis_worker, "get_queue_service", _QS)
