...
def test_data_depth_levels():
    ...
    for level in levels:
        ...
        result = toolkit.get_stock_fundamentals_unified.invoke(
            {"ticker": ticker, "start_date": "2025-06-01", "end_date": "2025-07-15", "curr_date": "2025-07-15"},
        )
        ...
        results[level] = {"data_length": data_length, "line_count": line_count, "sections": sections, "content": result}
    ...
    for level in levels:
        data = results[level]
        print(
            f"{level:<8} {level_names[level]:<8} {data['data_length']:,<12} {data['line_count']:<8} {len(data['sections']):<8}",
        )
    ...
    print("  📈 数据长度增长:")
    print(
        f"    - 级别1→3: {level3_length - level1_length:+,} 字符 ({((level3_length / level1_length - 1) * 100):+.1f}%)",
    )
    print(
        f"    - 级别3→5: {level5_length - level3_length:+,} 字符 ({((level5_length / level3_length - 1) * 100):+.1f}%)",
    )
    print(
        f"    - 级别1→5: {level5_length - level1_length:+,} 字符 ({((level5_length / level1_length - 1) * 100):+.1f}%)",
    )
