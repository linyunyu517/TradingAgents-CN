...
def test_enhanced_tool_binding():
    ...
    try:
        ...
        # 测试工具调用
        response = llm_with_tools.invoke([HumanMessage(content="请调用get_test_stock_data工具获取AAPL的股票数据")])
...
def test_tool_call_response_validation():
    ...
    try:
        ...
        # 测试工具调用修复
        broken_tool_call = {"function": {"name": "test_tool", "arguments": {"param1": "value1"}}}
...
def test_comprehensive_tool_calling():
    ...
    try:
        ...
        # 测试多轮对话和工具调用
        messages = [HumanMessage(content="请帮我分析苹果公司(AAPL)的股票，并获取相关新闻")]
