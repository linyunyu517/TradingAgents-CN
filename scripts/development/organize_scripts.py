...
def create_scripts_structure():
    ...
    script_categories = {
        ...
        "maintenance": {"description": "维护和管理脚本", "scripts": ["branch_manager.py", "version_manager.py"]},
        "development": {
            "description": "开发辅助脚本",
            "scripts": [
        ...
        "deployment": {
            "description": "部署和发布脚本",
            "scripts": ["create_github_release.py", "release_v0.1.2.py", "release_v0.1.3.py"],
        },
        "docker": {
...
"""
    ...
    for item in scripts_path.iterdir():
        if item.is_file() and item.suffix in [".py", ".sh", ".bat", ".js"] and item.name not in ["README.md"]:
