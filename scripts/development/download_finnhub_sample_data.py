...
def main():
    ...
    try:
        ...
        result = get_finnhub_news(ticker="AAPL", curr_date=datetime.now().strftime("%Y-%m-%d"), look_back_days=3)

        if result and "无法获取" not in result:
            logger.info("✅ 新闻数据获取成功！")
            logger.info(f"示例内容: {result[:200]}...")
        else:
            logger.error("⚠️ 新闻数据获取失败，请检查配置")
