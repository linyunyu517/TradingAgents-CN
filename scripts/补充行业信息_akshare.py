...
async def 补充行业信息(limit: int | None = None, batch_size: int = 50, delay: float = 0.5):
    """
    补充行业信息主函数
    ...
    try:
        # 1. 查询需要补充行业信息的股票
        query = {"$or": [{"industry": "未知"}, {"industry": {"$exists": False}}, {"industry": None}, {"industry": ""}]}
        ...
        for i, stock in enumerate(stocks, 1):
            ...
            try:
                ...
                if info["industry"] != "未知" or info["area"] != "未知":
                    # 更新数据库
                    update_data = {"industry": info["industry"], "area": info["area"], "updated_at": datetime.utcnow()}

                    result = await collection.update_one(
                        {"$or": [{"code": code}, {"symbol": code}]}, {"$set": update_data},
                    )
                    ...
                    if result.modified_count > 0:
                        ...
                        logger.warning("   ⚠️  未更新: 可能已存在相同数据")
                        skipped_count += 1
                else:
                    logger.warning("   ⚠️  未获取到有效信息")
                    failed_count += 1
        ...
        if remaining_count > 0:
            logger.info("💡 提示: 可以再次运行此脚本继续补充")
        else:
            logger.info("🎉 所有股票的行业信息已补充完成！")
...
示例:
  ...
  python scripts/补充行业信息_akshare.py --batch-size 10 --delay 1.0
    ...
    parser.add_argument("--limit", type=int, default=None, help="限制处理的股票数量（默认：全部）")
    parser.add_argument("--batch-size", type=int, default=50, help="每批处理的股票数量（默认：50）")
    parser.add_argument("--delay", type=float, default=0.5, help="每只股票之间的延迟（秒）（默认：0.5）")

    args = parser.parse_args()

    # 运行异步任务
    asyncio.run(补充行业信息(limit=args.limit, batch_size=args.batch_size, delay=args.delay))
