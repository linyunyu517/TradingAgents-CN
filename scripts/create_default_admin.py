...
if not mongo_password:
    ...
    sys.exit(1)
MONGO_URI = f"mongodb://admin:{mongo_password}@localhost:27017/tradingagentscn?authSource=admin"
...
def connect_mongodb() -> MongoClient:
    ...
    try:
        ...
        print("   请确保 MongoDB 容器正在运行")
        print("   运行: docker ps | grep mongodb")
        sys.exit(1)


def create_admin_user(db: any, username: str, password: str, email: str, overwrite: bool = False) -> bool:
    """创建管理员用户"""
...
def list_users(db: any):
    """列出所有用户"""
    users_collection = db.users
    users = list(users_collection.find({}, {"username": 1, "email": 1, "is_admin": 1, "is_active": 1, "created_at": 1}))
...
示例:
  ...
  python scripts/create_default_admin.py --list
    ...
    # 创建用户
    print()
    success = create_admin_user(db, args.username, args.password, args.email, args.overwrite)
