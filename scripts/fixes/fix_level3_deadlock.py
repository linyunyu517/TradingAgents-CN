...
# BUG-151: 使用 Path 计算项目根目录，避免硬编码路径
# BUG-165: 将 sys.path.insert 替换为基于 Path 的相对路径
project_root = Path(__file__).resolve().parent.parent.parent  # scripts/fixes/ -> 项目根
...
def apply_fundamentals_analyst_fix():
    ...
    # BUG-151: 使用相对路径代替硬编码的绝对路径
    fundamentals_file = str(project_root / "tradingagents" / "agents" / "analysts" / "fundamentals_analyst.py")
...
def apply_conditional_logic_fix():
    ...
    # BUG-151: 使用相对路径代替硬编码的绝对路径
    conditional_file = str(project_root / "tradingagents" / "graph" / "conditional_logic.py")

    # 读取原文件
    with open(conditional_file, encoding="utf-8") as f:
        content = f.read()
...
# BUG-165: 使用 Path 计算项目根目录，避免 sys.path.insert(0, ...) 模式
...
'''
    ...
    # BUG-151: 使用相对路径代替硬编码的绝对路径
    test_file = project_root / "test_level3_fix.py"
