...
def create_admin_user(client):
    ...
    try:
        ...
        # BUG-161: 强制要求 ADMIN_PASSWORD 环境变量，不再提供不安全的硬编码默认值
        ...
        if existing_admin:
            ...
            admin_user = {
                ...
                "updated_at": datetime.utcnow(),
                "last_login": None,
                "profile": {"display_name": "系统管理员", "bio": "TradingAgents-CN 系统管理员", "avatar_url": None},
                "preferences": {
                    "theme": "light",
                    "language": "zh-CN",
                    "timezone": "Asia/Shanghai",
                    "notifications": {"email": True, "push": True, "analysis_complete": True, "system_alerts": True},
                },
                "usage_stats": {
                    "total_analyses": 0,
                    "total_tokens_used": 0,
...
def create_web_user_config():
    ...
    try:
        ...
        # 创建用户配置文件
        # BUG-161: 强制从环境变量获取密码，不再提供硬编码默认值
...
def main():
    ...
    print("\n🔐 登录信息:")
    print("  用户名: admin")
    # BUG-161: 不再显示密码，提示用户使用环境变量 ADMIN_PASSWORD 设置的密码登录
    print("  密码: [使用 ADMIN_PASSWORD 环境变量设置的密码]")
