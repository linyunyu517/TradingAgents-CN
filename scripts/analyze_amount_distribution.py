    ...
    try:
        ...
        for label, min_val, max_val in ranges:
            if max_val == float("inf"):
                count = await view.count_documents({"source": "tushare", "amount": {"$gte": min_val}})
            else:
                count = await view.count_documents({"source": "tushare", "amount": {"$gte": min_val, "$lt": max_val}})
        ...
        for label, min_val, max_val in sample_ranges:
            ...
            async for doc in cursor:
                logger.info(
                    f"  {doc.get('code')} {doc.get('name'):10s}: "
