...
def find_tradingagents_container():
    ...
    container_names = ["TradingAgents-web", "tradingagents-web", "tradingagents_web_1", "tradingagents-cn_web_1"]
    ...
    success, output, _error = run_command("docker ps --format 'table {{.Names}}\t{{.Image}}\t{{.Status}}'")
    if success:
        print(output)
...
def get_container_info(container_name):
    ...
    # 容器镜像
    success, output, _error = run_command(f"docker inspect {container_name} --format '{{{{.Config.Image}}}}'")
    if success:
...
def explore_log_locations(container_name):
    ...
    for location in log_locations:
        ...
        # 查找日志文件
        success, output, _error = run_command(
        ...
        if success and output.strip():
            ...
            for log_file in log_files:
                ...
                # 获取文件信息
                success2, output2, _error2 = run_command(f"docker exec {container_name} ls -lh {log_file}")
                if success2:
...
def copy_log_files(container_name, log_files):
    ...
    for log_file in log_files:
        ...
        success, _output, error = run_command(f"docker cp {container_name}:{log_file} {local_file}")
        if success:
            print("   ✅ 复制成功")
