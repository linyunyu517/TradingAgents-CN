...
# 默认管理员用户
DEFAULT_ADMIN = {"username": "admin", "password": "admin123", "email": "admin@tradingagents.cn"}
...
def _build_unique_query(doc: dict[str, Any], collection_name: str) -> dict[str, Any] | None:
    """根据集合的唯一键构建查询条件"""
    if "_id" in doc:
        return {"_id": doc["_id"]}
...
def convert_to_bson(data: Any) -> Any:
    ...
    if isinstance(data, dict):
        ...
        for key, value in data.items():
            # 处理 ObjectId
            if (key == "_id" or key.endswith("_id")) and isinstance(value, str) and len(value) == 24:
                try:
...
def connect_mongodb(use_docker: bool = True, config: dict | None = None) -> MongoClient:
    """连接到 MongoDB
    ...
    try:
        ...
        if use_docker:
            print("   请确保在 Docker 容器内运行，或使用 --host 参数在宿主机运行")
            print("   检查容器: docker ps | grep mongodb")
        else:
...
def import_collection(
    db: Any, collection_name: str, documents: list[dict[str, Any]], overwrite: bool = False,
) -> dict[str, int]:
    """导入单个集合"""
...
示例:
  ...
  python scripts/import_config_and_create_user.py --create-user-only
    ...
    parser.add_argument(
        "export_file", nargs="?", help="导出的 JSON 文件路径（默认：install/database_export_config_*.json）",
    )
    parser.add_argument(
        "--host",
        action="store_true",
    ...
    parser.add_argument("--overwrite", action="store_true", default=True, help="覆盖已存在的数据（默认：覆盖）")
    parser.add_argument("--incremental", action="store_true", help="增量模式：跳过已存在的数据")
    parser.add_argument("--collections", nargs="+", help="指定要导入的集合（默认：所有配置集合）")
    parser.add_argument("--create-user-only", action="store_true", help="只创建默认用户，不导入数据")
    parser.add_argument("--skip-user", action="store_true", help="跳过创建默认用户")
    parser.add_argument("--mongodb-port", type=int, help="MongoDB 端口（覆盖 .env 配置）")
    parser.add_argument("--mongodb-host", type=str, help="MongoDB 主机（覆盖 .env 配置）")
    ...
    if not args.create_user_only:
        ...
        # 确定要导入的集合
        collections_to_import = args.collections or [c for c in CONFIG_COLLECTIONS if c in data]
    ...
    print("\n📝 后续步骤:")
    print("   1. 重启后端服务: docker restart tradingagents-backend")
    print("   2. 访问前端并使用默认账号登录")
    print("   3. 检查系统配置是否正确加载")
