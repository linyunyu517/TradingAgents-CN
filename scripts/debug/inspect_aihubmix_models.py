...
def summarize(models: list[dict[str, Any]], focus_providers: list[str], sample_limit: int) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "group_counts": {
            vendor: len(items) for vendor, items in sorted(grouped.items(), key=lambda kv: (-len(kv[1]), kv[0]))
        },
        "focus_samples": {},
        "other_samples": [
            str(item.get("model_id") or item.get("id") or "") for item in grouped.get("other", [])[:sample_limit]
        ],
    }
    ...
    suspicious_patterns = re.compile(r"(qwen|glm|deepseek|kimi|moonshot|doubao|minimax|mimo|step|ernie|hunyuan)", re.IGNORECASE)
