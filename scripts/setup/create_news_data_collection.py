...
async def create_news_collection():
    ...
    try:
        ...
        # 1. 唯一索引：防止重复新闻
        unique_index = [("url", 1), ("title", 1), ("publish_time", 1)]
        await collection.create_index(unique_index, unique=True, name="url_title_time_unique", background=True)
        ...
        # 5. 复合索引：股票+时间（最常用查询）
        await collection.create_index([("symbol", 1), ("publish_time", -1)], name="symbol_time_desc", background=True)
        logger.info("✅ 创建股票时间复合索引: symbol_time_desc")

        # 6. 复合索引：多股票+时间
        await collection.create_index([("symbols", 1), ("publish_time", -1)], name="symbols_time_desc", background=True)
        ...
        await collection.create_index(
            [("symbol", 1), ("category", 1), ("publish_time", -1)], name="symbol_category_time", background=True,
        )
        ...
        # 13. 文本搜索索引
        await collection.create_index(
            [("title", "text"), ("content", "text"), ("summary", "text")], name="text_search_index", background=True,
        )
        logger.info("✅ 创建文本搜索索引: text_search_index")

        # 14. 创建时间索引（用于数据管理）
        await collection.create_index([("created_at", -1)], name="created_at_desc", background=True)
    ...
    try:
        ...
        for news in sample_news:
            await collection.replace_one(
                {"url": news["url"], "title": news["title"], "publish_time": news["publish_time"]}, news, upsert=True,
            )
