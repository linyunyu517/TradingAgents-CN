...
def local_install():
    ...
    try:
        subprocess.run([str(python_path), "-m", "pip", "install", "--upgrade", "pip"], check=True, capture_output=True)
    ...
    try:
        result = subprocess.run([str(pip_path), "install", "-r", "requirements.txt"], capture_output=True, text=True)
        if result.returncode == 0:
...
def main():
    ...
    # 执行安装
    success = False
    success = docker_install() if choice == "1" else local_install()
