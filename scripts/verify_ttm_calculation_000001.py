...
async def main():
    ...
    financial_data = await db.stock_financial_data.find_one(
        {"code": "000001", "data_source": "tushare"}, sort=[("report_period", -1)],
    )
    ...
    # 按报告期排序（最新的在前）
    income_statements_sorted = sorted(income_statements, key=lambda x: x.get("end_date", ""), reverse=True)
    ...
    if q2_2025:
        ...
        if q3_2024 and base_period:
            ...
            print(
                f"2024Q4单季 = 2024年报 - 2024Q3累计 = {base_revenue:.2f} - {q3_2024_revenue:.2f} = {q4_2024_single:.2f} 亿元",
            )

            print("\n最近4个单季度:")
            print(f"  2024Q4: {q4_2024_single:.2f} 亿元")
            print("  2025Q1: (需要2025Q1数据)")
            print("  2025Q2: (需要2025Q1数据)")
