...
def get_log_file_info(container_name, log_file):
    ...
    # 最后修改时间
    success, output, _error = run_command(f"docker exec {container_name} stat -c '%y' {log_file}")
    if success:
...
def copy_log_file(container_name, log_file, local_path=None):
    ...
    success, _output, error = run_command(f"docker cp {container_name}:{log_file} {local_path}")
    if success:
...
def main():
    ...
    if len(log_files) == 1:
        ...
        selected_log = selected_logs[0] if len(selected_logs) == 1 else None
