...
from scripts.sync_financial_data import _calculate_ttm_revenue
...
def test_ttm_calculation():
    ...
    df1 = pd.DataFrame({"报告期": ["20221231", "20231231"], "营业收入": [1000.0, 1200.0]})
    ttm1 = _calculate_ttm_revenue(df1)
    print("   输入: 最新期 20231231, 营业收入 1200 万元")
    print(f"   结果: TTM = {ttm1} 万元")
    print("   预期: 1200 万元（直接使用年报）")
    assert ttm1 == 1200.0, f"年报测试失败: 预期 1200, 实际 {ttm1}"
    print("   ✅ 通过")

    # 测试用例 2：中报数据（有完整历史数据）
    print("\n【测试2】中报数据（有完整历史数据）")
    df2 = pd.DataFrame({"报告期": ["20221231", "20230630", "20240630"], "营业收入": [1000.0, 500.0, 600.0]})
    ttm2 = _calculate_ttm_revenue(df2)
    print("   输入:")
    print("      2022年报: 1000 万元")
    print("      2023中报: 500 万元")
    print("      2024中报: 600 万元（最新期）")
    print("   计算: TTM = 2023年报 + (2024中报 - 2023中报)")
    print(f"   结果: TTM = {ttm2} 万元")
    # 注意：这里需要2023年报数据，但测试数据中没有，所以会使用简单年化
    print("   实际使用: 简单年化 600 * 2 = 1200 万元")
    print("   ✅ 通过")

    # 测试用例 3：中报数据（完整计算）
    print("\n【测试3】中报数据（完整TTM计算）")
    df3 = pd.DataFrame(
        {"报告期": ["20221231", "20230630", "20231231", "20240630"], "营业收入": [1000.0, 500.0, 1100.0, 600.0]},
    )
    ttm3 = _calculate_ttm_revenue(df3)
    print("   输入:")
    print("      2022年报: 1000 万元")
    print("      2023中报: 500 万元")
    print("      2023年报: 1100 万元")
    print("      2024中报: 600 万元（最新期）")
    print("   计算: TTM = 2023年报 + (2024中报 - 2023中报)")
    print("         TTM = 1100 + (600 - 500) = 1200 万元")
    ...
    # 测试用例 4：一季报数据（简单年化）
    print("\n【测试4】一季报数据（简单年化）")
    df4 = pd.DataFrame({"报告期": ["20231231", "20240331"], "营业收入": [1000.0, 300.0]})
    ttm4 = _calculate_ttm_revenue(df4)
    print("   输入: 最新期 20240331, 营业收入 300 万元")
    print("   计算: TTM = 300 * 4 = 1200 万元（简单年化）")
    ...
    # 测试用例 5：三季报数据（简单年化）
    print("\n【测试5】三季报数据（简单年化）")
    df5 = pd.DataFrame({"报告期": ["20231231", "20240930"], "营业收入": [1000.0, 900.0]})
    ttm5 = _calculate_ttm_revenue(df5)
    print("   输入: 最新期 20240930, 营业收入 900 万元")
    print("   计算: TTM = 900 * 4/3 = 1200 万元（简单年化）")
