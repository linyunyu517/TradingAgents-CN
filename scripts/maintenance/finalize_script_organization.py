...
def finalize_script_organization():
    ...
    logger.info("📁 完成TradingAgentsCN脚本文件的最终整理")
    logger.info("=")
    ...
    # 创建必要的目录
    directories_to_create = ["deployment", "docker", "git"]
    ...
    readme_contents = {
        ...
        "git": {
            ...
            "scripts": ["upstream_git_workflow.sh - 上游Git工作流", "setup_fork_environment.sh - 设置Fork环境"],
        },
    }
...
"""
    ...
    logger.info("✅ 更新主README: scripts/README.md")
    ...
    for subdir in subdirs:
        ...
        if subdir_path.exists():
            script_files = [
                f for f in subdir_path.iterdir() if f.is_file() and f.suffix in [".py", ".ps1", ".sh", ".bat", ".js"]
            ]
    ...
    # 检查根级别剩余脚本
    root_scripts = [
        f for f in scripts_dir.iterdir() if f.is_file() and f.suffix in [".py", ".ps1", ".sh", ".bat", ".js"]
    ]
