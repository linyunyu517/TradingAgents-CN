    ...
    try:
        ...
        if count > 0:
            ...
            async for doc in cursor:
                logger.info(
                    f"  {doc.get('code')} {doc.get('name')}: pct_chg={doc.get('pct_chg')}, close={doc.get('close')}",
                )
        ...
        has_pct_chg = await view.count_documents({"pct_chg": {"$ne": None, "$exists": True}, "source": "tushare"})
