...
# 步骤4: 推送镜像
# BUG-152: 修正标题描述与实际推送目标一致（推送至 Docker Hub 而非 GitHub Container Registry）
echo -e "${YELLOW}步骤4: 推送镜像到Docker Hub...${NC}"
...
# BUG-150: 修正文件名 docker-compose.hub.yml → docker-compose.hub.nginx.yml
echo -e "${CYAN}  docker-compose -f docker-compose.hub.nginx.yml up -d${NC}"
echo ""
