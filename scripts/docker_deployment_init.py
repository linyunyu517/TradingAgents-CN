...
async def check_docker_services():
    ...
    try:
        ...
        # BUG-150: 修正文件名从 docker-compose.hub.yml → docker-compose.hub.nginx.yml
...
async def init_mongodb():
    ...
    try:
        ...
        # 连接数据库
        # BUG-153: 使用配置的 MongoDB 连接串而非硬编码
...
async def create_default_admin_user(db):
    ...
    try:
        ...
        if admin_user:
            ...
            # BUG-160: 不在日志中输出密码
            logger.info("   密码: [已隐藏，请查看 config/admin_password.json]")
...
async def create_system_config(db):
    ...
    try:
        ...
        for config in system_configs:
            db.system_config.replace_one({"key": config["key"]}, config, upsert=True)
    ...
    try:
        model_configs = [
            {
                ...
                "enabled": True,
                "is_default": True,
                "config": {"max_tokens": 8000, "temperature": 0.7},
        ...
        for config in model_configs:
            db.model_config.replace_one(
                {"provider": config["provider"], "model_name": config["model_name"]}, config, upsert=True,
            )
...
async def setup_admin_password():
    ...
    try:
        ...
        if not config_file.exists():
            ...
            default_config = {"password": default_password}
            with open(config_file, "w", encoding="utf-8") as f:
                json.dump(default_config, f, ensure_ascii=False, indent=2)
            logger.info("✅ 创建默认管理员密码配置（已保存至 config/admin_password.json）")
        else:
            with open(config_file, encoding="utf-8") as f:
                config = json.load(f)
                config.get("password", "admin123")
            # BUG-160: 不在日志中输出密码
            logger.info("✅ 当前管理员密码: [已隐藏]")
...
async def main():
    ...
    try:
        ...
        logger.info("\n🔐 登录信息:")
        logger.info("- 用户名: admin")
        # BUG-160: 不在日志中直接输出密码，引导用户查看配置文件
        logger.info("- 密码: 请查看 config/admin_password.json 文件")
        logger.info("\n⚠️  重要提醒:")
