...
class ServiceManager:
    ...
    def start_service(self, name: str, command: list[str], cwd: str | None = None) -> bool:
        """启动单个服务"""
        try:
            print(f"🚀 启动 {name}...")
...
def check_dependencies():
    ...
    required_packages = ["fastapi", "uvicorn", "motor", "redis", "pydantic", "python-jose", "passlib"]
...
def check_services():
    ...
    try:
        ...
        print("⚠️ Redis AUTH 失败（密码不匹配），尝试无密码连接...")
        try:
            r = redis.Redis(host=settings.REDIS_HOST, port=settings.REDIS_PORT, password=None, decode_responses=True)
            r.ping()
            print("✅ Redis 无密码连接正常（降级模式）")
        except Exception as e:
            print(f"❌ Redis 无密码连接也失败: {e}")
...
def main():
    ...
    api_success = manager.start_service(
        "FastAPI服务",
        [sys.executable, "-m", "uvicorn", "webapi.main:app", "--host", "0.0.0.0", "--port", "8000", "--reload"],
    ...
    worker_success = manager.start_service(
        "分析Worker", [sys.executable, "scripts/start_worker.py"], cwd=str(project_root),
    )
