...
class StockDataPreparer:
    ...
    def _validate_format(self, stock_code: str, market_type: str) -> StockDataPreparationResult:
        ...
        elif market_type == "美股" and not re.match(r"^[A-Z]{1,5}$", stock_code.upper()):
            return StockDataPreparationResult(
                is_valid=False,
                stock_code=stock_code,
                market_type="美股",
    ...
    def _trigger_data_sync_sync(self, stock_code: str, start_date: str, end_date: str) -> dict:
        ...
        try:
            ...
            return {"success": False, "message": f"同步失败: {e!s}", "synced_records": 0, "data_source": None}
        ...
        try:
            ...
            for data_source in priority_order:
                try:
                    ...
                    try:
                        rt_result = await service.sync_realtime_quotes(
                            symbols=[stock_code],
    ...
    def _prepare_us_stock_data(
        ...
        try:
            ...
            # US stock validation removed - only China A-shares via Tushare supported
            raise NotImplementedError("美股验证已移除，仅支持 A 股数据")
...
def prepare_stock_data(
    stock_code: str, market_type: str = "auto", period_days: int | None = None, analysis_date: str | None = None,
) -> StockDataPreparationResult:
    """
    便捷函数：预获取和验证股票数据
