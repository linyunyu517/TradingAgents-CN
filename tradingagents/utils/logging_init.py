...
def init_logging(config_override: dict | None = None) -> None:
    ...
    """
    # BUG-NEW-008 修复: 强制 stdout/stderr 使用 UTF-8 编码，防止 GBK 乱码
    # 设置环境变量确保 Python IO 使用 UTF-8（影响子进程）
...
class _SessionAdapter(Logger):
    """会话适配器，注入 session_id 到日志记录"""

    def __init__(self, logger: Logger, session_id: str) -> None:
        self.logger = logger
...
def get_session_logger(session_id: str, module_name: str = "session") -> Logger:
    """
    获取会话专用日志器
    """
    return _SessionAdapter(get_logger(module_name), session_id)
