...
class StructuredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # 保留 exc_info / traceback
        if record.exc_info and record.exc_info[0] is not None:
            import traceback

            log_entry["exc_info"] = "".join(traceback.format_exception(*record.exc_info)).strip()
        elif record.exc_text:
            log_entry["exc_info"] = record.exc_text.strip()
...
class TradingAgentsLogger:
    ...
    def _convert_toml_config(self, toml_config: dict[str, Any]) -> dict[str, Any]:
        ...
        # 检查Docker环境
        is_docker = os.getenv("DOCKER_CONTAINER") == "true" or logging_config.get("docker", {}).get("enabled", False)
        ...
        return {
            ...
            "docker": {"enabled": is_docker, "stdout_only": logging_config.get("docker", {}).get("stdout_only", True)},
    ...
    def _setup_logging(self):
        ...
        # 🔧 BUG-014 Fix: 不直接清除已有处理器，避免清除 app 的日志配置
        # 如果已有处理器（可能由 app.core.logging_config.setup_logging 设置），
        # 则跳过添加新处理器，仅配置特定 logger 级别，让两种日志系统共存
    ...
    def _add_structured_handler(self, logger: logging.Logger):
        ...
        structured_handler = logging.handlers.RotatingFileHandler(
            log_file, maxBytes=self._parse_size("10MB"), backupCount=3, encoding="utf-8",
        )
    ...
    def log_module_start(
        self, logger: logging.Logger, module_name: str, stock_symbol: str, session_id: str, **extra_data,
    ):
        """记录模块开始分析"""
        logger.info(
