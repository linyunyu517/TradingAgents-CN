...
_SYSTEM_SETTINGS_WARNED = False

# ========== BUG-016 修复: 确保 .env 文件在模块加载时尽早加载 ==========
try:
    from dotenv import load_dotenv

    load_dotenv()
except ImportError:
    _logger.debug("python-dotenv 未安装，跳过 .env 文件加载")
except Exception as e:
    _logger.debug(f"加载 .env 文件失败（不影响运行）: {e}")
...
def _get_system_settings_sync() -> dict:
    ...
    ========== BUG-016 修复: ==========
    - 当前实现因事件循环冲突，动态配置获取被禁用
    - 配置优先级: DB(system_settings) > ENV > default
    - 当 DB 不可用时，实际生效为: ENV > default
    - 调用方会发出 warning 日志，便于运维人员了解动态配置状态

    ========== [FIX] 2026-06-18: Fix 2.7 - 增强日志说明 ==========
    - 动态配置通过 MongoDB 的 system_settings 集合管理
    - 当前同步上下文无法安全调用异步 DB 查询（事件循环冲突）
    - 如需启用动态配置，请将应用迁移到 async 上下文中调用 config_provider
    - 临时变通方案: 通过 .env 文件设置环境变量覆盖默认值
    """
    # 临时解决方案：完全禁用动态配置获取，避免事件循环冲突
    # TODO: 未来可以考虑使用线程池或其他方式来安全获取动态配置
    # ========== BUG-012 修复: 首次 warning 提醒，后续 debug 降噪 ==========
    global _SYSTEM_SETTINGS_WARNED
    if not _SYSTEM_SETTINGS_WARNED:
        _logger.warning(
            "🔧 [动态配置] DB 动态配置获取已禁用（同步上下文无法安全调用异步 DB 查询）。"
            "配置优先级: DB > ENV > default，当前生效: ENV > default。"
            "如需覆盖，请在 .env 文件中设置对应环境变量。",
        )
        _SYSTEM_SETTINGS_WARNED = True
    else:
        _logger.debug("🔧 [动态配置] DB 动态配置获取已禁用（已提醒，不再重复警告）")
    return {}
...
def get_number(env_var: str, system_key: str | None, default: float, caster: Callable[[Any], Any]) -> float | int:
    ...
    if system_key:
        ...
        if isinstance(eff, dict) and system_key in eff:
            return _coerce(eff.get(system_key), caster, default)  # type: ignore[no-any-return]

    # 2) 环境变量
    env_val = os.getenv(env_var)
    if env_val is not None and str(env_val).strip() != "":
        return _coerce(env_val, caster, default)  # type: ignore[no-any-return]
...
def get_int(env_var: str, system_key: str | None, default: int) -> int:
    return cast("int", get_number(env_var, system_key, default, int))
...
def use_app_cache_enabled(default: bool = False) -> bool:
    ...
    try:
        ...
        _logger.warning("[runtime_settings] 日志记录失败（已安全跳过）", exc_info=True)
    return val
...
def get_timezone_name(default: str = "Asia/Shanghai") -> str:
    ...
    try:
        ...
        _logger.warning("[runtime_settings] 获取系统设置失败（已安全跳过）", exc_info=True)
