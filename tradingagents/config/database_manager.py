    ...
    def _detect_redis(self) -> tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                # [FIX 2026-06-18 P1] 如果带密码连接失败，尝试无密码连接
                if "AUTH" in str(ping_err) or "NOAUTH" in str(ping_err) or "Authentication" in str(ping_err):
                    self.logger.warning(f"[DB] [FIX P1] ⚠️ Redis AUTH 验证失败，尝试无密码回退: {ping_err}")
                    connect_kwargs_noauth = {k: v for k, v in connect_kwargs.items() if k != "password"}
                    client = redis.Redis(**connect_kwargs_noauth)
                    client.ping()
                else:
                    raise  # 非 AUTH 错误继续抛出
    ...
    def _initialize_connections(self):
        ...
        if self.mongodb_available:
            try:
                ...
                # 构建连接参数
                # ========== BUG-015 修复: 添加 maxPoolSize 连接池配置 ==========
                connect_kwargs = {
                    "host": self.mongodb_config["host"],
                    "port": self.mongodb_config["port"],
                    "serverSelectionTimeoutMS": self.mongodb_config["server_selection_timeout"],
                    "connectTimeoutMS": self.mongodb_config["connect_timeout"],
                    "socketTimeoutMS": self.mongodb_config["socket_timeout"],
                    "maxPoolSize": self.mongodb_config.get("max_pool_size", 100),
                    "minPoolSize": self.mongodb_config.get("min_pool_size", 10),
                    "maxIdleTimeMS": 30000,
                }
                ...
                self.logger.info(f"MongoDB客户端初始化成功 (连接池: {connect_kwargs.get('maxPoolSize', 'default')})")
            except Exception as e:
                # [FIX 2026-06-18 P1] 优雅降级: MongoDB 不可用时仅记录警告，不阻止启动
                self.logger.warning(f"[DB] [FIX P1] ⚠️ MongoDB 客户端初始化失败，降级到 file 存储后端: {e}")
                self.mongodb_available = False
        ...
        if self.redis_available:
            try:
                ...
                # [FIX 2026-06-18 P1] 优雅降级: Redis 不可用时仅记录警告，不阻止启动
                self.logger.warning(f"[DB] [FIX P1] ⚠️ Redis 客户端初始化失败，降级到 file 缓存后端: {e}")
                self.redis_available = False
