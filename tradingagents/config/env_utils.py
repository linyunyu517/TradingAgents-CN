...
def parse_bool_env(env_var: str, default: bool = False) -> bool:
    ...
    # 真值列表
    true_values = {"true", "1", "yes", "on", "enable", "enabled", "t", "y", "ok", "okay"}

    # 假值列表
    false_values = {"false", "0", "no", "off", "disable", "disabled", "f", "n", "none", "null", "nil"}
...
def get_env_info(env_var: str) -> _EnvInfo:
    """
    获取环境变量的详细信息
...
def validate_required_env_vars(required_vars: list[str]) -> _ValidationResult:
    """
    验证必需的环境变量是否已设置
    ...
    Returns:
        dict: 验证结果
    """
    results: _ValidationResult = {"all_set": True, "missing": [], "empty": [], "valid": []}
