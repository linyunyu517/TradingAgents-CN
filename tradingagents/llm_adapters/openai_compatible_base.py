        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_retryable and attempt < max_retries - 1:
                    ...
                    jitter = random.uniform(0, 0.5)  # 增加随机抖动，避免雪崩
                    wait_time = delay + jitter
                    logger.warning(
                        f"⚠️ [{getattr(self, '_provider_name', 'unknown')}] "
                        f"API请求失败 (尝试 {attempt + 1}/{max_retries}), "
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not api_key:
            ...
            qianfan_api_key = env_api_key if env_api_key and is_valid_api_key(env_api_key) else None
        else:
