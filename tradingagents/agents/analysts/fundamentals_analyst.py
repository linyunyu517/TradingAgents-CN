...
# 【优化】导入数学预处理层 (Phase 1【器】)
from tradingagents.agents.analysts.fundamentals_math import run_full_math_layer
...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        max_tool_calls = 3  # 与 conditional_logic.py 保持一致

        # 2. 检查是否已有报告 - 如果有则跳过 LLM 调用
        existing_report = state.get("fundamentals_report", "")
        if existing_report and len(existing_report) > 100:
            logger.info(f"📊 [基本面分析师] ✅ 报告已存在 (长度: {len(existing_report)}), 跳过节点")
            return {
                "fundamentals_report": existing_report,
        ...
        if tool_call_count >= max_tool_calls:
            logger.warning(f"🔧 [BUG-001] 工具调用已达上限 {max_tool_calls}，强制结束")
            fallback = (
                f"基本面分析（股票代码：{state['company_of_interest']}）\n\n"
                f"由于达到最大工具调用次数限制，使用简化分析模式。"
            )
            return {
        ...
        # 获取公司名称
        company_name = get_company_name(ticker, market_info, "基本面分析师")
        ...
        debug_tool_names = get_tool_names(tools)
        ...
        # ===== 【优化】Phase 1【器】数学预处理层（在所有路径前执行） =====
        math_features = run_full_math_layer(ticker, current_date)
        math_summary = math_features.get("summary", "")
        math_context_block = ""
        if math_features.get("data_available"):
            try:
                import json
                math_context_block = f"""

【数学层分析特征】
{json.dumps(math_features, ensure_ascii=False, indent=2, default=str)}
"""
            except Exception:
                math_context_block = f"\n【数学层概要】{math_summary}"
            logger.info(f"[基础面分析师] ✅ 数学层注入完成: {math_summary}")
        else:
            logger.info("[基础面分析师] ⚠️ 数学层数据不足，跳过特征注入")
            math_context_block = f"\n【数学层说明】部分数据源不可用，依赖原始数据进行分析。"
        # ===== 数学预处理层结束 =====

        # 🔧 消息清理：移除孤立的 tool_calls，防止 DeepSeek API 400 错误
        cleaned_messages = clean_orphaned_tool_calls(state["messages"])
        logger.info(f"[基本面分析师] 消息清理完成: {len(state['messages'])}→{len(cleaned_messages)} 条")
        ...
        if current_tool_calls > 0:
            ...
            if has_tool_result:
                ...
                logger.info(f"[基本面分析师-强制报告] 消息清理完成: {len(messages)}→{len(cleaned_messages)} 条")

                logger.info("🔧 [强制生成报告] 使用专门的提示词重新调用LLM...")
                # 🛡️ [H14 集中化防御] 使用 safe_chain_invoke 自动清理孤儿 tool_calls
                force_result = safe_chain_invoke(force_chain, {"messages": cleaned_messages})

                report = safe_extract_content(force_result)
                if not report:
                    report = "基本面分析完成"
        ...
        # BUG-001: 不要重新赋值 tool_call_count，使用函数顶层的变量
        current_tool_count = sum(1 for msg in messages if isinstance(msg, ToolMessage))
        logger.info(f"🔍 [统计] 历史工具调用次数: {tool_call_count}")

        logger.info(
            f"🔍 [重复调用检查] 汇总 - 工具结果数: {current_tool_count}, 已有工具结果: {has_tool_result}, 已有分析内容: {has_analysis_content}",
        )
        ...
        if has_tool_result or has_analysis_content:
            ...
            # BUG-001: 使用函数顶层的 tool_call_count，非局部变量
            return {
                "fundamentals_report": report,
                "messages": [result],
                "fundamentals_tool_call_count": tool_call_count,
        ...
        # BUG-NEW-001 修复: 预先检查消息历史中是否有数据源全部失败的迹象
        # 当所有数据源都失败时，避免浪费 LLM token 调用
        _all_sources_failed = False
        for _m in messages:
            if isinstance(_m, ToolMessage) and hasattr(_m, "content") and _m.content:
                if "所有数据源都无法获取" in str(_m.content):
                    _all_sources_failed = True
                    break
        if _all_sources_failed:
            logger.warning("🚫 [BUG-NEW-001] 检测到所有数据源均失败，跳过强制工具调用，使用兜底报告")
            fallback_report = (
                f"📊 基本面分析报告（{company_name} - {ticker}）\n\n"
                f"⚠️ **数据源异常说明**\n"
                f"当前所有配置的数据源（AKShare/BaoStock/Tushare/MongoDB）均无法获取数据，"
                f"请检查网络连接或数据源配置。\n\n"
                f"**建议操作：**\n"
                f"1. 检查数据源 API Key 是否有效\n"
                f"2. 确认网络连接是否正常\n"
                f"3. 稍后重试分析\n"
                f"4. 联系管理员检查数据源状态"
            )
            logger.info(f"✅ [BUG-NEW-001] 返回兜底报告，长度: {len(fallback_report)}字符")
            return {
                "fundamentals_report": fallback_report,
                "fundamentals_tool_call_count": tool_call_count,
                "messages": [result] if not isinstance(result, AIMessage) else [result],
            }
        ...
        try:
            ...
            if unified_tool:
                ...
                combined_data = invoke_with_timeout(
                    ...
                    timeout=60,
                    timeout_msg="基本面分析工具 get_stock_fundamentals_unified 数据获取",
                )
        ...
        # 生成基于真实数据的分析报告（提示词已精简 - 数学层负责分析，LLM负责翻译）
        analysis_prompt = f"""基于以下真实数据和数学层特征，对{company_name}（{ticker}）做基本面判断：
...
要求：使用中文，简洁专业。"""
        ...
        try:
            ...
            # 🛡️ [H14 集中化防御] 使用 safe_chain_invoke（无 messages 键时保持原行为）
            analysis_result = safe_chain_invoke(analysis_chain, {"analysis_request": analysis_prompt})

            report = safe_extract_content(analysis_result)
            if not report:
        ...
        return {"fundamentals_report": report, "fundamentals_tool_call_count": tool_call_count}
        ...
        return {
            "messages": [result],
            "fundamentals_report": safe_extract_content(result) or str(result),
            "fundamentals_tool_call_count": tool_call_count,
        }
