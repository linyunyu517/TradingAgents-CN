...
# 导入Google工具调用处理器
# 🛡️ [H14 集中化防御] 导入消息清理工具、安全包装器和公共工具函数
...
def create_china_market_analyst(llm, toolkit):
    ...
    def china_market_analyst_node(state):
        ...
        # 获取公司名称
        company_name = get_company_name(ticker, market_info, "中国市场分析师")
        logger.info(f"[中国市场分析师] 公司名称: {company_name}")

        # 中国股票分析工具
        tools = [
            toolkit.get_stock_market_data_unified,  # 统一数据工具（替代已移除的 get_china_stock_data）
...
确保在报告末尾附上Markdown表格总结关键发现和投资建议。"""
        ...
        result = safe_chain_invoke(chain, {"messages": state["messages"]})
...
请基于当前市场环境和政策背景，提供专业的股票筛选建议。"""
        ...
        return {
            "messages": [result],
            "stock_screening_report": safe_extract_content(result),
