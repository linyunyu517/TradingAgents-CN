    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        if mongodb_client:
            try:
                ...
                for collection_name in ["stock_data", "news_data", "fundamentals_data"]:
                    if collection_name in db.list_collection_names():
                        ...
                        try:
                            ...
                            self.logger.warning(f"获取集合 {collection_name} 统计信息失败（已安全跳过）", exc_info=True)
                ...
                self.logger.warning("获取 MongoDB 缓存统计失败（已安全跳过）", exc_info=True)
        ...
        if redis_client:
            try:
                ...
                self.logger.warning("获取 Redis 统计信息失败（已安全跳过）", exc_info=True)
        ...
        if self.primary_backend == "file" or self.fallback_enabled:
            for pkl_file in self.cache_dir.glob("*.pkl"):
                try:
                    ...
                    self.logger.warning(f"读取缓存文件大小失败（已安全跳过）: {pkl_file.name}", exc_info=True)
