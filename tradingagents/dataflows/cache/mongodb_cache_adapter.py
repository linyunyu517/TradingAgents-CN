...
class MongoDBCacheAdapter:
    ...
    def __init__(self):
        ...
        self.db = None
        # ========== BUG-013 修复: 添加缓存未命中互斥锁，防止缓存穿透 ==========
        self._cache_miss_locks: dict[str, threading.Lock] = {}
        self._cache_miss_locks_lock = threading.Lock()
    ...
    # ========== BUG-013 修复: 添加缓存穿透防护方法 ==========
    def _get_cache_miss_lock(self, key: str) -> threading.Lock:
        """获取或创建针对特定缓存键的互斥锁"""
        with self._cache_miss_locks_lock:
            if key not in self._cache_miss_locks:
                self._cache_miss_locks[key] = threading.Lock()
            return self._cache_miss_locks[key]

    def _acquire_cache_lock(self, key: str) -> bool:
        """尝试获取缓存锁，成功返回 True，失败（其他线程正在回填）返回 False"""
        lock = self._get_cache_miss_lock(key)
        acquired = lock.acquire(blocking=False)
        if not acquired:
            logger.debug(f"⏳ 缓存回填进行中，等待其他线程完成: {key}")
            # 阻塞等待，直到持有锁的线程完成
            lock.acquire()
            lock.release()
            return False  # 告诉调用者去缓存中重新读取
        return True

    def _release_cache_lock(self, key: str) -> None:
        """释放缓存锁"""
        lock = self._get_cache_miss_lock(key)
        lock.release()

    def _init_mongodb_connection(self):
        """初始化MongoDB连接（带真实连通性测试）"""
        try:
            ...
            if self.mongodb_client:
                ...
                # 🔴 P0 FIX: 真实 ping 测试，而非仅检查 db is not None
                self.db.command("ping")
                logger.debug("✅ MongoDB连接初始化成功（ping通过）")
            else:
    ...
    def _get_data_source_priority(self, symbol: str) -> list:
        ...
        # 默认顺序：Tushare
        logger.info("📊 [数据源优先级] 使用默认顺序: ['tushare']")
        return ["tushare"]
    ...
    # [FIX] 2026-06-18: Fix 2.7c - 添加重试机制和优雅降级
    def get_news_data(
        self, symbol: str | None = None, hours_back: int = 24, limit: int = 20, max_retries: int = 2,
        ...
        Args:
            ...
            hours_back: 向后查询的小时数
            limit: 最大返回条数
            max_retries: 连接失败时的最大重试次数（默认 2）

        Returns:
            Optional[List[Dict]]: 新闻列表，失败返回 None
        """
        if not self.use_app_cache or self.db is None:
            return None
        ...
        for attempt in range(1 + max_retries):
            try:
                ...
                if attempt < max_retries:
                    wait = (attempt + 1) * 0.5  # 0.5s, 1s 退避
                    logger.warning(f"⚠️ [MongoDB-新闻] 第{attempt + 1}次查询失败: {e}，{wait:.1f}s后重试...")
                    ...
                    logger.warning(f"⚠️ [数据来源: MongoDB-新闻数据] 获取新闻数据失败（已重试{max_retries}次）: {e}")

        return None

    def get_social_media_data(
        self, symbol: str | None = None, hours_back: int = 24, limit: int = 20,
    ) -> list[dict[str, Any]] | None:
        """获取社媒数据"""
        if not self.use_app_cache or self.db is None:
            return None
