...
class BaseStockDataProvider(ABC):
    ...
    async def get_historical_data(
        self, symbol: str, start_date: str | date, end_date: str | date | None = None,
    ) -> pd.DataFrame | None:
        """
        获取历史数据
    ...
    def standardize_basic_info(self, raw_data: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        # 基础标准化逻辑
        # [Loop3-M1] 对 code / name / symbol 添加 str() 强制类型转换，确保返回值类型一致
        return {
            "code": str(raw_data.get("code", raw_data.get("symbol", ""))),
            "name": str(raw_data.get("name", "")),
            "symbol": str(raw_data.get("symbol", raw_data.get("code", ""))),
