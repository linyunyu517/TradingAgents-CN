...
def _get_tushare_snapshot(symbol: str) -> dict[str, float | None]:
    try:
        try:
            ...
            logger.debug("[fund_snapshot] tushare 模块不可用，跳过 tushare snapshot")
            return {}
        ...
        try:
            ...
            logger.warning("获取 ROE 数据失败（已安全跳过）", exc_info=True)
        return {
            "pe": pe,
            "pb": pb,
            "market_cap": mv,  # 单位：万元
...
def get_cn_fund_snapshot(symbol: str) -> dict[str, float | None]:
    """
