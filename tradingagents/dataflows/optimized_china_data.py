...
成交量: {volume}"""
    ...
    def _generate_report_template(
        ...
        """根据模板级别生成基本面分析报告（合并 4 个重复模板为一个基方法）

        Args:
            template_name: 模板级别 ("basic", "standard", "full", "detailed", "comprehensive")
            symbol: 股票代码
            company_name: 公司名称
            current_price: 当前价格
            change_pct: 涨跌幅
            volume: 成交量
            industry_info: 行业信息 dict
            financial_estimates: 财务指标 dict
        ...
        Returns:
            str: 格式化后的报告文本
        """
        now_str = datetime.now(ZoneInfo(get_timezone_name())).strftime("%Y-%m-%d %H:%M:%S")
        date_cn = datetime.now(ZoneInfo(get_timezone_name())).strftime("%Y年%m月%d日")

        # ---- 标题 ----
        if template_name == "basic":
            title = f"# 中国A股基本面分析报告 - {symbol} (基础版)"
        elif template_name in ("detailed", "comprehensive"):
            title = f"# 中国A股基本面分析报告 - {symbol} (全面版)"
        else:
            title = f"# 中国A股基本面分析报告 - {symbol}"
...
- **成交量**: {volume}"""
        basic_info += f"""
- **分析日期**: {date_cn}{data_source_note}

"""

        # ---- 财务数据 ----
        if template_name == "basic":
            financial_section = f"""## 💰 核心财务指标
...
"""
        ...
        # ---- 行业分析 ----
        if template_name in ("detailed", "comprehensive"):
            industry_section = f"""## 📈 行业分析
...
**数据来源**: {data_source or "多源数据"}数据接口 + 基本面分析模型
**生成时间**: {now_str}
"""
        else:
            footer = f"""---
**重要声明**: 本报告基于公开数据和模型估算生成，仅供参考，不构成投资建议。
**数据来源**: {data_source or "多源数据"}数据接口
**生成时间**: {now_str}
"""
...
"""
        ...
        report = self._generate_report_template(
            template_name=analysis_modules,
            symbol=symbol,
    ...
    def _get_industry_info(self, symbol: str) -> dict:
        ...
        market_info = market_map.get(code_prefix, {"market": "未知市场", "exchange": "未知交易所", "type": "综合"})
    ...
    def _estimate_financial_metrics(self, symbol: str, current_price: str) -> dict:
        """获取真实财务指标（从 MongoDB 获取，失败则返回估算值）"""
        ...
        # 如果无法获取真实数据，返回错误标记
        error_msg = f"无法获取股票 {symbol} 的财务数据。MongoDB 获取失败。"
        logger.error(f"❌ {error_msg}")
    ...
    def _run_async_safely(self, coro, timeout=60):
        """安全地运行异步协程，兼容有/无事件循环的场景
        ...
        try:
            ...
            if loop.is_running():
                # 事件循环正在运行（如 FastAPI/Notebook 环境），在新线程中执行
    ...
    def _get_real_financial_metrics(self, symbol: str, price_value: float) -> dict:
        ...
        try:
            ...
            # 唯一数据源：从 MongoDB stock_financial_data 集合获取标准化财务数据
            from tradingagents.config.runtime_settings import use_app_cache_enabled

            if use_app_cache_enabled(False):
                logger.info(f"🔍 从 MongoDB stock_financial_data 集合获取{symbol}财务数据")
                ...
                if financial_data:
                    ...
                    if metrics:
                        logger.info("✅ MongoDB 财务数据解析成功，返回指标")
                        # 设置数据来源标识
                        metrics["data_source"] = "数据库"
                        return metrics
                    logger.warning("⚠️ MongoDB 财务数据解析失败，返回估算指标")
                else:
                    logger.info(f"🔄 MongoDB 未找到{symbol}财务数据，返回估算指标")
            else:
                logger.info("🔄 数据库缓存未启用，返回估算指标")

        except Exception as e:
            logger.debug(f"获取{symbol}真实财务数据失败: {e}")

        # Tushare is the primary data source — all BaoStock fallback code removed
        ...
        try:
            ...
            if tushare and tushare.connected:
                tushare_fin = self._run_async_safely(tushare.get_financial_data(code6))
                if tushare_fin:
                    logger.info("✅ [Tushare Fallback] 从 Tushare 获取 %s 财务数据", code6)
                    metrics = {
                        "eps": float(tushare_fin.get("eps", 0) or 0),
                        "roe": float(tushare_fin.get("roe", 0) or 0),
                        "roa": float(tushare_fin.get("roa", 0) or 0),
                        "gross_margin": float(tushare_fin.get("gross_margin", 0) or 0),
                        "net_margin": float(tushare_fin.get("net_margin", 0) or 0),
                        "total_assets": float(tushare_fin.get("total_assets", 0) or 0),
                        "total_liab": float(tushare_fin.get("total_liab", 0) or 0),
                        "total_equity": float(tushare_fin.get("total_equity", 0) or 0),
                        "fundamental_score": 5,
                        "valuation_score": 5,
                        "growth_score": 5,
                        "risk_level": "中等",
            ...
            logger.debug(f"Tushare 财务数据兜底失败: {e}")
    ...
    def _calculate_fundamental_score(self, metrics: dict, stock_info: dict) -> float:
        ...
        if roe_str != "N/A":
            try:
                ...
                logger.debug("ROE转换失败: %s", roe_str)
        ...
        if net_margin_str != "N/A":
            try:
                ...
                logger.debug("净利率转换失败: %s", net_margin_str)
    ...
    def _calculate_risk_level(self, metrics: dict, stock_info: dict) -> str:
        ...
        if debt_ratio_str != "N/A":
            try:
                ...
                logger.debug("负债率转换失败: %s", debt_ratio_str)

        # 根据行业判断
        industry = stock_info.get("industry", "")
        if "银行" in industry:
            return "中等"
        if "科技" in industry or "创业板" in industry:
            return "较高"
...
- 风险承受能力较低的投资者应避免"""
    ...
    def _try_get_old_cache(self, symbol: str, start_date: str, end_date: str) -> str | None:
        ...
        try:
            ...
            logger.warning("中国数据优化处理失败", exc_info=True)
    ...
    def _get_cached_raw_financial_data(self, symbol: str) -> dict:
        ...
        try:
            ...
            if not client:
                logger.debug("📊 [财务缓存] MongoDB客户端不可用")
                return None
            ...
            financial_doc = stock_financial_collection.find_one(
                {"$or": [{"symbol": symbol}, {"code": symbol}]}, sort=[("updated_at", -1)],
            )
            ...
            if financial_doc:
                ...
                # 提取各类财务数据
                # 第一优先级：检查 raw_data 字段（同步服务使用的原始数据结构）
            ...
            cache_doc = collection.find_one(
                {"symbol": symbol, "cache_type": "raw_financial_data"}, sort=[("updated_at", -1)],
            )
    ...
    def _cache_raw_financial_data(self, symbol: str, financial_data: dict, stock_info: dict):
        ...
        try:
            ...
            # 使用upsert更新或插入
            collection.replace_one({"symbol": symbol, "cache_type": "raw_financial_data"}, cache_doc, upsert=True)
...
# 注意：_add_financial_cache_methods() 已移除，四个缓存方法已作为正式实例方法
# 直接定义在 OptimizedChinaDataProvider 类中（见上）
