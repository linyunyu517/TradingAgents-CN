    ...
    if not st.session_state.analysis_results:
        try:
            ...
            if latest_id:
                ...
                if progress_data and progress_data.get("status") == "completed" and "raw_results" in progress_data:
                    # 恢复分析结果
...
def check_frontend_auth_cache():
    ...
    if st.session_state.get("authenticated", False):
        ...
        if not auth_manager.is_authenticated() and st.session_state.get("user_info"):
            ...
            try:
                auth_manager.login_user(st.session_state.user_info, st.session_state.get("login_time", time.time()))
...
def main():
    ...
    if not auth_manager.is_authenticated():
        ...
        if (
            ...
            and st.session_state.get("user_info")
            and st.session_state.get("login_time")
        ):
            logger.info("🔄 从session state恢复认证状态")
            try:
                auth_manager.login_user(st.session_state.user_info, st.session_state.login_time)
                logger.info(
                    f"✅ 成功从session state恢复用户 {st.session_state.user_info.get('username', 'Unknown')} 的认证状态",
                )
            except Exception as e:
    ...
    # 添加调试按钮（仅在调试模式下显示）
    if os.getenv("DEBUG_MODE") == "true" and st.button("🔄 清除会话状态"):
    ...
    default_show_guide = not (
        st.session_state.get("analysis_running", False) or st.session_state.get("analysis_results") is not None
    )
    ...
    with col1:
        ...
        if form_data.get("submitted", False) and not st.session_state.get("analysis_running", False):
            ...
            if not is_valid:
                ...
                # 创建进度回调函数
                def progress_callback(message: str, step: int | None = None, total_steps: int | None = None):
        ...
        if current_analysis_id:
            ...
            # 显示进度（根据状态决定是否显示刷新控件）
            progress_col1, _progress_col2 = st.columns([4, 1])
            with progress_col1:
                st.markdown("### 📊 分析进度")
        ...
        should_show_results = (analysis_results and not analysis_running and current_analysis_id) or (
