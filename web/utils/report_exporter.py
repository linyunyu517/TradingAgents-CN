...
"""
        ...
        analysis_modules = [
            ...
            ("diffusion_report", "🔄 扩散模型预测", "扩散模型DDIM采样的量化决策分析"),
            ("fusion_report", "🔗 融合决策报告", "AIF/扩散/交易员决策的加权融合结果"),
        ]
        ...
        for engine_info in pdf_engines:
            ...
            try:
                ...
                try:
                    ...
                    logger.debug("清理 PDF 临时文件失败: %s", output_file if "output_file" in locals() else "unknown")
...
"""
    ...
    def export_report(self, results: dict[str, Any], format_type: str) -> bytes | None:
        ...
        try:
            ...
            logger.error(f"❌ 导出失败: {e!s}", exc_info=True)
            st.error(f"❌ 导出失败: {e!s}")
            return None
...
def save_modular_reports_to_results_dir(results: dict[str, Any], stock_symbol: str) -> dict[str, str]:
    ...
    try:
        ...
        # [Bug 6 修复] 标准化股票代码用于目录路径：去掉 .SS/.SZ/.HK 等交易所后缀
        clean_symbol = re.sub(r"\.(SS|SZ|HK|BO|TO|L|AX)$", "", stock_symbol, flags=re.IGNORECASE)
        logger.info(f"[路径标准化] stock_symbol='{stock_symbol}' -> clean_symbol='{clean_symbol}'")

        # 创建股票专用目录
        analysis_date = datetime.now().strftime("%Y-%m-%d")
        stock_dir = results_dir / clean_symbol / analysis_date
        ...
        # 同时保存到MongoDB
        logger.info("🔍 [MongoDB调试] 开始MongoDB保存流程")
        ...
        if MONGODB_REPORT_AVAILABLE and mongodb_report_manager:
            ...
            try:
                ...
                if reports_content:
                    ...
                    success = mongodb_report_manager.save_analysis_report(
                        stock_symbol=stock_symbol, analysis_results=results, reports=reports_content,
                    )

                    if success:
                        logger.info("✅ 分析报告已同时保存到MongoDB")
                    else:
                        logger.warning("⚠️ MongoDB保存失败，但文件保存成功")
                else:
                    logger.warning("⚠️ 没有报告内容可保存到MongoDB")
            ...
            logger.warning(
                f"⚠️ MongoDB保存跳过 - AVAILABLE: {MONGODB_REPORT_AVAILABLE}, Manager: {mongodb_report_manager is not None}",
            )
    ...
    with col1:
        if st.button("📄 导出 Markdown", help="导出为Markdown格式"):
            ...
            if content:
                ...
                st.download_button(label="📥 下载 Markdown", data=content, file_name=filename, mime="text/markdown")
            else:
        ...
        if st.button("📝 导出 Word", help="导出为Word文档格式"):
            ...
            with st.spinner("正在生成Word文档，请稍候..."):
                try:
                    ...
                    logger.error(f"❌ [EXPORT] Word导出异常: {e!s}")
                    logger.error(f"❌ Word导出异常: {e!s}", exc_info=True)
                    st.error(f"❌ Word文档生成失败: {e!s}")
            ...
            with st.spinner("正在生成PDF，请稍候..."):
                try:
                    ...
                    if content:
                        ...
                        st.download_button(
                            label="📥 下载 PDF", data=content, file_name=filename, mime="application/pdf",
                        )
                    else:
                        logger.error("❌ PDF导出失败，content为空")
                        st.error("❌ PDF生成失败")
                except Exception as e:
                    logger.error(f"❌ PDF导出异常: {e!s}", exc_info=True)
                    st.error("❌ PDF生成失败")
...
def save_analysis_report(
    stock_symbol: str, analysis_results: dict[str, Any], report_content: str | None = None,
) -> bool:
    """
    保存分析报告到MongoDB
    ...
    try:
        ...
        # 调用MongoDB报告管理器保存报告
        # 将报告内容包装成字典格式
        reports_dict = {"markdown": report_content, "generated_at": datetime.now().strftime("%Y-%m-%d %H:%M:%S")}

        success = mongodb_report_manager.save_analysis_report(
            stock_symbol=stock_symbol, analysis_results=analysis_results, reports=reports_dict,
        )
        ...
        logger.error(f"❌ 保存分析报告到MongoDB时发生异常 - 股票: {stock_symbol}, 错误: {e!s}")
        return False
