...
### 🔴 激进风险分析师观点
{risky_analysis or "暂无激进风险分析"}

### 🟡 中性风险分析师观点
{neutral_analysis or "暂无中性风险分析"}

### 🟢 保守风险分析师观点
{safe_analysis or "暂无保守风险分析"}

### 🏛️ 风险管理委员会最终决议
{judge_decision or "暂无风险管理决议"}
...
def run_stock_analysis(
    ...
    if TOKEN_TRACKING_ENABLED:
        ...
        if isinstance(estimated_cost_result, tuple):
            estimated_cost, _currency = estimated_cost_result
        else:
    ...
    try:
        ...
        logger_manager.log_analysis_complete(
            logger, stock_symbol, "comprehensive_analysis", session_id, analysis_duration, total_cost,
        )
        ...
        try:
            ...
            # 2. 保存分析报告到MongoDB
            logger.info("🗄️ [MongoDB保存] 开始保存分析报告到MongoDB")
            save_success = save_analysis_report(stock_symbol=stock_symbol, analysis_results=results)

            if save_success:
                logger.info("✅ [MongoDB保存] 分析报告已成功保存到MongoDB")
                update_progress("✅ 分析报告已保存到数据库和本地文件")
            else:
                logger.warning("⚠️ [MongoDB保存] MongoDB报告保存失败")
                if local_files:
            ...
            logger.error(f"❌ [报告保存] 保存分析报告时发生错误: {save_error!s}")
        ...
        logger_manager.log_module_error(
            logger, "comprehensive_analysis", stock_symbol, session_id, analysis_duration, str(e),
        )
...
def generate_demo_results_deprecated(
    stock_symbol, analysis_date, analysts, research_depth, llm_provider, llm_model, error_msg, market_type="美股",
):
    """
    已弃用：生成演示分析结果
...
*注意: 这是演示数据*
    ...
    confidence = demo_decision["confidence"]
