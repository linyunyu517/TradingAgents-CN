...
class MongoDBReportManager:
    ...
    def _create_indexes(self):
        ...
        try:
            # 创建复合索引
            self.collection.create_index([("stock_symbol", 1), ("analysis_date", -1), ("timestamp", -1)])
    ...
    def save_analysis_report(
        self, stock_symbol: str, analysis_results: dict[str, Any], reports: dict[str, str],
    ) -> bool:
        """保存分析报告到MongoDB"""
        ...
        try:
            ...
            # [Bug 6 修复] 标准化 stock_symbol 用于 MongoDB 查询（去掉交易所后缀）
            import re
            ...
            document = {
                ...
                "stock_symbol": clean_symbol,  # 使用标准化代码
            ...
            # [Bug 6 修复] 使用 update_one + upsert 避免同一股票代码重复插入
            filter_criteria = {"stock_symbol": clean_symbol, "analysis_date": timestamp.strftime("%Y-%m-%d")}
            # 若存在则覆盖（更新 reports 和 updated_at），否则插入新文档
            result = self.collection.update_one(filter_criteria, {"$set": document}, upsert=True)

            if result.upserted_id or result.modified_count > 0:
                logger.info(f"✅ 分析报告已保存到MongoDB: {analysis_id} (stock_symbol={clean_symbol})")
                return True
            logger.error("❌ MongoDB保存失败")
            return False
    ...
    def get_report_by_id(self, analysis_id: str) -> dict[str, Any] | None:
        """根据ID获取单个分析报告"""
        if not self.connected:
            return None
    ...
    def fix_inconsistent_reports(self) -> bool:
        ...
        try:
            ...
            query = {"$or": [{"reports": {"$exists": False}}, {"reports": {}}, {"reports": None}]}
            ...
            for doc in inconsistent_docs:
                try:
                    ...
                    update_data = {"$set": {"reports": {}, "updated_at": datetime.now()}}

                    result = self.collection.update_one({"_id": doc["_id"]}, update_data)
    ...
    def save_report(self, report_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            result = self.collection.replace_one({"analysis_id": report_data["analysis_id"]}, report_data, upsert=True)
