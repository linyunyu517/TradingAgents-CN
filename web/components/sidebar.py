...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        llm_provider = st.selectbox(
            ...
            in ["dashscope", "deepseek", "google", "openai", "openrouter", "siliconflow", "custom_openai", "qianfan"]
            else 0,
        ...
        if llm_provider == "dashscope":
            ...
            llm_model = st.selectbox(
                ...
                index=current_index,
                format_func=lambda x: {"deepseek-chat": "DeepSeek Chat - 通用对话模型，适合股票分析"}[x],
            ...
            qianfan_options = ["ernie-3.5-8k", "ernie-4.0-turbo-8k", "ERNIE-Speed-8K", "ERNIE-Lite-8K"]
            ...
            openai_options = ["gpt-4o", "gpt-4o-mini", "gpt-4-turbo", "gpt-4", "gpt-3.5-turbo"]
            ...
            **🔧 支持的服务类型:**
            ...
            model_category = st.selectbox(
                ...
                index=["openai", "anthropic", "meta", "google", "custom"].index(st.session_state.model_category)
                if st.session_state.model_category in ["openai", "anthropic", "meta", "google", "custom"]
                else 0,
                format_func=lambda x: {
                    "openai": "🤖 OpenAI (GPT系列)",
            ...
            if model_category == "openai":
                ...
                default_value = (
                    st.session_state.custom_model or "anthropic/claude-3.7-sonnet"
                )
        ...
        with st.expander("⚙️ 高级设置"):
            enable_memory = st.checkbox("启用记忆功能", value=False, help="启用智能体记忆功能（可能影响性能）")

            enable_debug = st.checkbox("调试模式", value=False, help="启用详细的调试信息输出")

            max_tokens = st.slider(
                "最大输出长度", min_value=1000, max_value=8000, value=4000, step=500, help="AI模型的最大输出token数量",
            )
        ...
        def validate_api_key(key, expected_format):
            ...
            if (
                (expected_format == "dashscope" and key.startswith("sk-") and len(key) >= 32)
                or (expected_format == "deepseek" and key.startswith("sk-") and len(key) >= 32)
                or (expected_format == "finnhub" and len(key) >= 20)
                or (expected_format == "tushare" and len(key) >= 32)
                or (expected_format == "google" and key.startswith("AIza") and len(key) >= 32)
                or (expected_format == "openai" and key.startswith("sk-") and len(key) >= 40)
                or (expected_format == "anthropic" and key.startswith("sk-") and len(key) >= 40)
                or (expected_format == "reddit" and len(key) >= 10)
            ):
