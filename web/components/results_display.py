...
def render_analysis_info(results):
    ...
    with st.expander("📋 分析配置信息", expanded=False):
        ...
        with col1:
            ...
            provider_name = {"dashscope": "阿里百炼", "google": "Google AI", "qianfan": "文心一言（千帆）"}.get(
        ...
        with col3:
            ...
            st.metric(label="分析师数量", value=f"{analysts_count}个", help="参与分析的AI分析师数量")
...
def render_decision_summary(decision, stock_symbol=None):
    ...
    with col1:
        ...
        {"BUY": "normal", "SELL": "inverse", "HOLD": "off", "买入": "normal", "卖出": "inverse", "持有": "off"}.get(
            action.upper(), "normal",
        )
    ...
    with col2:
        ...
        st.metric(label="置信度", value=confidence_str, delta=confidence_delta, help="AI对分析结果的置信度")
    ...
    with col3:
        ...
        st.metric(label="风险评分", value=risk_str, delta=risk_delta, delta_color="inverse", help="投资风险评估分数")
...
def render_detailed_analysis(state):
    ...
    # 过滤出有数据的模块
    available_modules = []
    reports_sub = state.get("reports", {}) if isinstance(state, dict) else {}
    for module in analysis_modules:
        # 检查 key 是否存在：优先顶层 state，其次 reports 子字典
        module_data = None
        if module["key"] in state:
            module_data = state[module["key"]]
    ...
    for _i, (tab, module) in enumerate(zip(tabs, available_modules, strict=False)):
        with tab:
            # 在内容区域显示图标和描述
...
def create_price_chart(price_data):
    ...
    fig.add_trace(
        go.Scatter(
            x=price_data["date"], y=price_data["price"], mode="lines", name="股价", line=dict(color="#1f77b4", width=2),
        ),
    )

    # 设置图表样式
    fig.update_layout(
        title="股价走势图", xaxis_title="日期", yaxis_title="价格 ($)", hovermode="x unified", showlegend=True,
    )
