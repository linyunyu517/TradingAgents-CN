...
安全加固：
- [Bug B02] 移除 fallback 中的硬编码密码 admin123/user123
- [Bug B21] 移除弱认证回退
"""
...
try:
    ...
    from ..utils.auth_manager import auth_manager as imported_auth_manager
    ...
    try:
        ...
        from web.utils.auth_manager import auth_manager as imported_auth_manager
        ...
        try:
            ...
            try:
                ...
                from auth_manager import auth_manager as imported_auth_manager

                auth_manager = imported_auth_manager
            except ImportError:
                # 🔒 [Bug B02/B21] 不再使用硬编码密码回退
                # 如果所有导入都失败，允许用户访问但显示配置错误
                st.error("""
                ⚠️ **系统配置错误：认证模块加载失败**

                请确保：
                1. 项目已正确安装：`pip install -e .`
                2. 环境变量 `ADMIN_DEFAULT_PASSWORD` 和 `USER_DEFAULT_PASSWORD` 已设置
                3. 运行 `python scripts/generate_credentials.py` 生成安全凭据
                """)
                auth_manager = None
    ...
    with st.container():
        ...
        with col2:
            ...
            username = st.text_input(
                "用户名", placeholder="请输入您的用户名", key="username_input", label_visibility="collapsed",
            )
            ...
            if st.button("🚀 立即登录", use_container_width=True, key="login_button"):
                if auth_manager is None:
                    ...
                    运行 `python scripts/generate_credentials.py` 生成安全凭据
                    """)
...
def render_sidebar_user_info():
    ...
    # 获取用户角色的中文显示
    role_display = {"admin": "管理员", "user": "普通用户"}.get(user_info.get("role", "user"), "用户")
