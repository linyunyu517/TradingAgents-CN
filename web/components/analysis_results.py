...
def render_analysis_results():
    ...
    with st.sidebar:
        ...
        analyst_filter = st.selectbox(
            ...
            help="社交媒体分析师现已支持A股（使用东方财富股吧数据源）、港股和美股",
        )
        ...
        selected_tags = st.multiselect("🏷️ 标签过滤", sorted(all_tags)) if all_tags else []

    # 加载分析结果
    results = load_analysis_results(
        start_date=start_date,
        end_date=end_date,
        stock_symbol=stock_filter or None,
...
def render_results_charts(results: list[dict[str, Any]]):
    ...
    if daily_results:
        ...
        fig_line.update_layout(title="每日分析趋势", xaxis_title="日期", yaxis_title="分析数量", hovermode="x unified")
...
def render_results_comparison(results: list[dict[str, Any]]):
    ...
    with col1:
        st.write("**分析结果 A**")
        selected_a = st.selectbox("选择第一个分析结果", result_options, format_func=lambda x: x[0], key="compare_a")
        result_a = results[selected_a[1]]

    with col2:
        st.write("**分析结果 B**")
        selected_b = st.selectbox("选择第二个分析结果", result_options, format_func=lambda x: x[0], key="compare_b")
        result_b = results[selected_b[1]]
    ...
    comparison_fields = [
        ...
        ("diffusion_report", "🔄 扩散模型预测"),
        ("fusion_report", "🔗 融合决策报告"),
    ]

    # 创建对比标签页
    available_fields = []
    for field_key, field_name in comparison_fields:
        if (result_a.get(field_key)) or (result_b.get(field_key)):
            available_fields.append((field_key, field_name))

    if available_fields:
        tabs = st.tabs([field_name for _, field_name in available_fields])

        for i, (tab, (field_key, field_name)) in enumerate(zip(tabs, available_fields, strict=False)):
            with tab:
                col1, col2 = st.columns(2)
    ...
    if result_options:
        ...
        if performance:
            ...
            for i, (key, value) in enumerate(performance.items()):
                with perf_cols[i]:
                    st.metric(
                        key.replace("_", " ").title(), f"{value:.2f}" if isinstance(value, (int, float)) else str(value),
                    )
...
def render_detailed_analysis_content(selected_result):
    ...
    if selected_result.get("reports"):
        ...
        if len(tab_names) == 1:
            ...
            for i, (tab, report_key) in enumerate(zip(tabs, report_tabs, strict=False)):
                with tab:
    ...
    analysis_modules = [
        ...
        {"key": "risk_assessment", "title": "⚠️ 风险评估", "icon": "⚠️", "description": "风险因素识别、风险等级评估"},
        {"key": "investment_plan", "title": "📋 投资建议", "icon": "📋", "description": "具体投资策略、仓位管理建议"},
        {
        ...
        {
            "key": "diffusion_report",
            "title": "🔄 扩散模型预测",
            "icon": "🔄",
            "description": "扩散模型 DDIM 采样的量化决策分析",
        },
        {
            "key": "fusion_report",
            "title": "🔗 融合决策报告",
            "icon": "🔗",
            "description": "AIF/扩散/交易员决策的加权融合结果",
        },
    ]

    # 过滤出有数据的模块
    available_modules = []
    reports_sub = selected_result.get("reports", {}) if isinstance(selected_result, dict) else {}
    for module in analysis_modules:
        # 检查 key 是否存在：优先顶层 selected_result，其次 reports 子字典
        module_data = None
        if module["key"] in selected_result:
            module_data = selected_result[module["key"]]
        elif module["key"] in reports_sub:
            module_data = reports_sub[module["key"]]

        if module_data:
            # 检查字典类型的数据是否有实际内容
            if isinstance(module_data, dict):
                has_content = any(v for v in module_data.values() if v)
                if has_content:
                    available_modules.append(module)
            elif isinstance(module_data, str) and len(module_data.strip()) > 10:
                available_modules.append(module)
            elif module_data:
                # 其他非空类型
                available_modules.append(module)
    ...
    if not available_modules:
        ...
        if selected_result.get("full_data"):
        ...
        if analysis_data:
            ...
            for i, (tab, (key, value)) in enumerate(zip(tabs, tab_data, strict=False)):
                with tab:
    ...
    for i, (tab, module) in enumerate(zip(tabs, available_modules, strict=False)):
        with tab:
            # 在内容区域显示图标和描述
    ...
    with st.container():
        ...
        ("fusion_report", "🔗 融合决策报告"),
                    ...
                    if available_reports:
                        ...
                        for _i, (tab, (field_key, field_name, content)) in enumerate(
                            zip(tabs, available_reports, strict=False),
                        ):
                            with tab:
                                if isinstance(content, str):
                                    st.markdown(content)
        ...
        if len(tab_names) == 1:
            ...
            for _i, (tab, report_key) in enumerate(zip(tabs, report_tabs, strict=False)):
                with tab:
                    st.markdown(reports[report_key])
