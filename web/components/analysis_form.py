...
def render_analysis_form():
    ...
    with st.form("analysis_form", clear_on_submit=False):
        ...
        with col1:
            ...
            market_type = st.selectbox(
                "选择市场 🌍", options=market_options, index=market_index, help="选择要分析的股票市场",
            )
            ...
            # 分析日期
            analysis_date = st.date_input("分析日期 📅", value=datetime.date.today(), help="选择分析的基准日期")
        ...
        with col1:
            market_analyst = st.checkbox(
                "📈 市场分析师", value="market" in cached_analysts, help="专注于技术面分析、价格趋势、技术指标",
            )

            # 社交媒体分析师 - 所有市场类型均支持（A股使用东方财富股吧数据源）
            social_analyst = st.checkbox(
                "💭 社交媒体分析师",
                value="social" in cached_analysts,
                help="分析社交媒体情绪、投资者情绪指标（A股使用东方财富股吧数据源）",
            )

        with col2:
            news_analyst = st.checkbox(
                "📰 新闻分析师", value="news" in cached_analysts, help="分析相关新闻事件、市场动态影响",
            )

            fundamentals_analyst = st.checkbox(
                "💰 基本面分析师", value="fundamentals" in cached_analysts, help="分析财务数据、公司基本面、估值水平",
            )
        ...
        with st.expander("🔧 高级选项"):
            include_sentiment = st.checkbox("包含情绪分析", value=True, help="是否包含市场情绪和投资者情绪分析")

            include_risk_assessment = st.checkbox("包含风险评估", value=True, help="是否包含详细的风险因素评估")
    ...
    if submitted and stock_symbol:  # 确保有股票代码才提交
        ...
        return form_data
    if submitted and not stock_symbol:
        # 用户点击了提交但没有输入股票代码
