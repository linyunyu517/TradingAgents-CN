...
def render_logs_charts(logs: list[dict[str, Any]]):
    ...
    if action_types:
        fig_pie = px.pie(values=list(action_types.values()), names=list(action_types.keys()), title="操作类型分布")
    ...
    if daily_logs:
        ...
        fig_line.update_layout(title="每日操作趋势", xaxis_title="日期", yaxis_title="操作数量")
...
def log_operation(username: str, action_type: str, action: str, details: dict | None = None, success: bool = True):
    """记录操作日志"""
    try:
        logs_dir = get_operation_logs_dir()
