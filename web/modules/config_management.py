...
from tradingagents.config.config_manager import ModelConfig, PricingConfig, config_manager
...
def render_config_management():
    ...
    page = st.sidebar.selectbox("选择功能", ["模型配置", "定价设置", "使用统计", "系统设置"])
...
def render_model_config():
    ...
    if models:
        ...
        if selected_model_idx is not None:
            ...
            env_has_key = env_status["api_keys"].get(model.provider.lower(), False)
            if env_has_key:
                st.info("💡 此模型的API密钥来自 .env 文件，修改 .env 文件后需重启应用生效")

            col1, col2 = st.columns(2)

            with col1:
                new_api_key = st.text_input(
                    "API密钥", value=model.api_key, type="password", key=f"edit_api_key_{selected_model_idx}",
                )
                if env_has_key:
                ...
                new_temperature = st.slider(
                    "温度参数", 0.0, 2.0, model.temperature, 0.1, key=f"edit_temperature_{selected_model_idx}",
                )

            with col2:
                new_enabled = st.checkbox("启用模型", value=model.enabled, key=f"edit_enabled_{selected_model_idx}")
                new_base_url = st.text_input(
                    "自定义API地址 (可选)", value=model.base_url or "", key=f"edit_base_url_{selected_model_idx}",
                )
    ...
    with col1:
        new_provider = st.selectbox(
            "供应商", ["dashscope", "openai", "google", "anthropic", "other"], key="new_provider",
        )
    ...
    with col2:
        new_max_tokens = st.number_input(
            "最大Token数", value=4000, min_value=1000, max_value=32000, key="new_max_tokens",
        )
...
def render_pricing_config():
    ...
    with col2:
        new_input_price = st.number_input(
            "输入价格 (每1K token)", min_value=0.0, step=0.001, format="%.6f", key="new_pricing_input",
        )
        new_output_price = st.number_input(
            "输出价格 (每1K token)", min_value=0.0, step=0.001, format="%.6f", key="new_pricing_output",
        )
...
def render_usage_statistics():
    ...
    if records:
        ...
        if daily_stats:
            ...
            fig.add_trace(go.Scatter(x=dates, y=costs, mode="lines+markers", name="每日成本 (¥)", yaxis="y"))

            fig.add_trace(go.Scatter(x=dates, y=requests, mode="lines+markers", name="每日请求数", yaxis="y2"))
...
def render_system_settings():
    ...
    with col1:
        default_provider = st.selectbox(
            ...
            index=["dashscope", "openai", "google", "anthropic"].index(settings.get("default_provider", "dashscope")),
            key="settings_default_provider",
        )

        enable_cost_tracking = st.checkbox(
            "启用成本跟踪", value=settings.get("enable_cost_tracking", True), key="settings_enable_cost_tracking",
        )

        currency_preference = st.selectbox(
            "首选货币",
            ["CNY", "USD", "EUR"],
            index=["CNY", "USD", "EUR"].index(settings.get("currency_preference", "CNY")),
            key="settings_currency_preference",
        )

    with col2:
        default_model = st.text_input(
            "默认模型", value=settings.get("default_model", "qwen-turbo"), key="settings_default_model",
        )
    ...
    auto_save_usage = st.checkbox(
        "自动保存使用记录", value=settings.get("auto_save_usage", True), key="settings_auto_save_usage",
    )
    ...
    with st.expander("🔑 API密钥详细状态", expanded=False):
        ...
        with api_col1:
            ...
            for provider, configured in env_status["api_keys"].items():
                if provider in ["dashscope", "deepseek", "openai", "google", "anthropic"]:
...
def main():
    """主函数"""
    st.set_page_config(page_title="配置管理 - TradingAgents", page_icon="⚙️", layout="wide")
