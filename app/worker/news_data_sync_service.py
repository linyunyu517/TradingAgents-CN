...
try:
    ...
    logging.warning("⚠️ get_tushare_provider 不可用（模块已删除）")
...
class NewsDataSyncService:
    ...
    async def sync_stock_news(
        self, symbol: str, data_sources: list[str] | None = None, hours_back: int = 24, max_news_per_source: int = 50,
    ) -> NewsSyncStats:
        """
        同步单只股票的新闻数据
        ...
        try:
            ...
            if "tushare" in data_sources:
                try:
                    tushare_news = await self._sync_tushare_news(symbol, hours_back, max_news_per_source)
                    if tushare_news:
            ...
            if "realtime" in data_sources:
                try:
                    realtime_news = await self._sync_realtime_news(symbol, hours_back, max_news_per_source)
                    if realtime_news:
            ...
            if all_news:
                ...
                # 批量保存
                saved_count = await news_service.save_news_data(unique_news, "multi_source", "CN")
    ...
    async def _sync_tushare_news(self, symbol: str, hours_back: int, max_news: int) -> list[dict[str, Any]]:
        """同步Tushare新闻"""
        try:
            ...
            news_data = await provider.get_stock_news(symbol=symbol, limit=max_news, hours_back=hours_back)
    ...
    async def _sync_realtime_news(self, symbol: str, hours_back: int, max_news: int) -> list[dict[str, Any]]:
        """同步实时新闻"""
        try:
            aggregator = await self._get_realtime_aggregator()

            # 获取实时新闻
            news_items = aggregator.get_realtime_stock_news(symbol, hours_back, max_news)
    ...
    def _classify_news_category(self, title: str) -> str:
        ...
        if any(word in title_lower for word in ["政策", "央行", "监管", "法规"]):
            return "policy_news"
        if any(word in title_lower for word in ["市场", "行情", "指数", "板块"]):
            return "market_news"
        if any(word in title_lower for word in ["研报", "分析", "评级", "推荐"]):
    ...
    def _assess_importance(self, title: str) -> str:
        ...
        if any(word in title_lower for word in high_importance_words):
            return "high"
        if any(word in title_lower for word in medium_importance_words):
            return "medium"
        return "low"
    ...
    async def sync_market_news(
        self, data_sources: list[str] | None = None, hours_back: int = 24, max_news_per_source: int = 100,
    ) -> NewsSyncStats:
        """
        同步市场新闻
        ...
        try:
            ...
            if "realtime" in data_sources:
                try:
                    ...
                    # 获取市场新闻（不指定股票代码）
                    news_items = aggregator.get_realtime_stock_news(None, hours_back, max_news_per_source)
            ...
            if all_news:
                ...
                # 批量保存
                saved_count = await news_service.save_news_data(unique_news, "market_news", "CN")
                stats.successful_saves = saved_count
                stats.failed_saves = len(unique_news) - saved_count
