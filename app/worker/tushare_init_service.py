...
class TushareInitService:
    ...
    async def run_full_initialization(
        ...
        self.stats = InitializationStats(started_at=datetime.utcnow(), total_steps=total_steps)
        ...
        try:
            ...
            self.stats.errors.append({"step": self.stats.current_step, "error": str(e), "timestamp": datetime.utcnow()})
            return self._get_initialization_summary()
    ...
    async def _step_initialize_news_data(self, historical_days: int):
        ...
        try:
            ...
            result = await self.sync_service.sync_news_data(hours_back=hours_back, max_news_per_stock=20)
...
async def run_tushare_full_initialization(historical_days: int = 365, skip_if_exists: bool = True):
    """APScheduler任务：运行完整的Tushare数据初始化"""
    try:
        service = await get_tushare_init_service()
        result = await service.run_full_initialization(historical_days=historical_days, skip_if_exists=skip_if_exists)
