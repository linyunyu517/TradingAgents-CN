...
async def save_internal_messages(request: InternalMessageBatchRequest):
    ...
    try:
        ...
        return ok(data=result, message=f"成功保存 {result['saved']} 条内部消息")

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"保存内部消息失败: {e!s}")
...
async def query_internal_messages(request: InternalMessageQueryRequest):
    ...
    try:
        ...
        return ok(
            data={"messages": messages, "count": len(messages), "params": request.dict()},
            message=f"查询到 {len(messages)} 条内部消息",
        )

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"查询内部消息失败: {e!s}")
...
async def get_latest_messages(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取最新消息失败: {e!s}")
...
async def search_messages(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"搜索消息失败: {e!s}")
...
async def get_research_reports(
    ...
    try:
        ...
        return ok(
            data={"reports": reports, "count": len(reports), "symbol": symbol, "department": department},
            message=f"获取到 {len(reports)} 份研究报告",
        )

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取研究报告失败: {e!s}")
...
async def get_analyst_notes(
    ...
    try:
        ...
        return ok(
            data={"notes": notes, "count": len(notes), "symbol": symbol, "author": author},
            message=f"获取到 {len(notes)} 条分析师笔记",
        )

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"获取分析师笔记失败: {e!s}")
    ...
    try:
        ...
        return ok(
            data={
                ...
                "symbol": symbol,
                "time_range": {"start_time": start_time, "end_time": end_time, "hours_back": hours_back},
            },
            message="统计信息获取成功",
        )
...
async def get_message_types():
    """获取支持的消息类型列表"""
    message_types = [
        {"code": "research_report", "name": "研究报告", "description": "深度研究分析报告"},
        {"code": "insider_info", "name": "内幕信息", "description": "内部获得的重要信息"},
        {"code": "analyst_note", "name": "分析师笔记", "description": "分析师的观点和笔记"},
        {"code": "meeting_minutes", "name": "会议纪要", "description": "重要会议的记录"},
        {"code": "internal_analysis", "name": "内部分析", "description": "内部团队的分析结果"},
    ]

    return ok(data={"message_types": message_types, "count": len(message_types)}, message="消息类型列表获取成功")
...
async def get_categories():
    """获取支持的分类列表"""
    categories = [
        {"code": "fundamental_analysis", "name": "基本面分析", "description": "公司基本面相关分析"},
        {"code": "technical_analysis", "name": "技术分析", "description": "技术指标和图表分析"},
        {"code": "market_sentiment", "name": "市场情绪", "description": "市场情绪和投资者行为分析"},
        {"code": "risk_assessment", "name": "风险评估", "description": "投资风险评估和管理"},
    ]

    return ok(data={"categories": categories, "count": len(categories)}, message="分类列表获取成功")
...
async def health_check():
    ...
    try:
        ...
        return ok(
            data={"status": "healthy", "total_messages": count, "service": "internal_message_service"},
            message="内部消息服务运行正常",
        )
