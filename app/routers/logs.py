...
async def read_log_file(request: LogReadRequest, current_user: dict = Depends(get_current_user)):
    """
    读取日志文件内容
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"读取日志文件失败: {e!s}")


@router.post("/export")
async def export_logs(request: LogExportRequest, current_user: dict = Depends(get_current_user)):
    """
    导出日志文件
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"导出日志文件失败: {e!s}")
...
async def delete_log_file(filename: str, current_user: dict = Depends(get_current_user)):
    """
    删除日志文件
