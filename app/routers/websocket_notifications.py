...
class ConnectionManager:
    ...
    def __init__(self):
        ...
        # [BUG-1 FIX] 添加事件循环 ID 隔离检测
        self._loop = None

    def _get_loop_safe(self):
        """安全获取当前事件循环，返回 (loop, is_same)"""
        try:
            current_loop = asyncio.get_running_loop()
            if self._loop is None:
                self._loop = current_loop
            return current_loop, self._loop is current_loop
        except RuntimeError:
            return None, False
    ...
    async def send_personal_message(self, message: dict, user_id: str):
        """发送消息给指定用户的所有连接"""
        # [BUG-1 FIX] 检查事件循环一致性，避免跨循环崩溃
        _, is_same_loop = self._get_loop_safe()
        if not is_same_loop:
            logger.warning(f"⚠️ [BUG-1] send_personal_message 跨事件循环调用: user={user_id}, 尝试安全处理")
        ...
        try:
            ...
            # [BUG-1 FIX] 捕获跨事件循环 RuntimeError，防止 1012 崩溃
            logger.error(f"❌ [BUG-1 FIX] 获取连接时事件循环错误: {e}, user={user_id}")
            return
        ...
        for connection in connections:
            try:
                ...
                # [BUG-1 FIX] 捕获跨事件循环异常
                logger.error(f"❌ [BUG-1 FIX] 发送消息事件循环错误: {e}, user={user_id}")
                # 不标记为死连接，避免二次触发锁操作
        ...
        if dead_connections:
            try:
                ...
                logger.error(f"❌ [BUG-1 FIX] 清理死连接时事件循环错误: {e}")
    ...
    async def broadcast(self, message: dict):
        ...
        _, is_same_loop = self._get_loop_safe()
        if not is_same_loop:
            logger.warning("⚠️ [BUG-1] broadcast 跨事件循环调用，尝试安全处理")
        ...
        try:
            ...
            logger.error(f"❌ [BUG-1 FIX] broadcast 获取连接时事件循环错误: {e}")
            return
        ...
        for connection in all_connections:
            try:
                ...
                logger.error(f"❌ [BUG-1 FIX] broadcast 发送事件循环错误: {e}")
            except Exception as e:
    ...
    async def send_heartbeat():
        while True:
            try:
                ...
                logger.debug(f"💓 [WS-Task] 心跳发送失败: {e}")
                break
...
async def send_task_progress_via_websocket(task_id: str, progress_data: dict, user_id: str | None = None):
    """
    通过 WebSocket 发送任务进度

    [BUG-5 FIX] 改为只推送给 task_id 关联的用户，而非广播给所有用户

    Args:
        task_id: 任务 ID
        progress_data: 进度数据
        user_id: 任务所属用户 ID。为 None 时尝试从 progress_data 中提取
    """
    # [BUG-5 FIX] 优先使用显式传递的 user_id，其次从 progress_data 提取
    if user_id is None:
    ...
    if user_id:
        # [BUG-5 FIX] 只推送给任务所属用户
        await manager.send_personal_message(message, user_id)
        logger.debug(f"📤 [WS-Task] 推送进度给 user={user_id}, task={task_id}")
    else:
        # 没有 user_id 时 fallback 到 broadcast 并记录警告
        logger.warning(f"⚠️ [BUG-5] send_task_progress_via_websocket 未指定 user_id，回退到 broadcast (task={task_id})")
