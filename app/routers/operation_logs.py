...
async def get_operation_logs(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取操作日志列表失败: {e!s}")


@router.get("/stats", response_model=OperationLogStatsResponse)
async def get_operation_log_stats(
    days: int = Query(30, ge=1, le=365, description="统计天数"), current_user: dict = Depends(get_current_user),
):
    """获取操作日志统计"""
    try:
        ...
        return OperationLogStatsResponse(success=True, data=stats, message="获取操作日志统计成功")

    except Exception as e:
        logger.error(f"获取操作日志统计失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取操作日志统计失败: {e!s}")


@router.get("/{log_id}")
async def get_operation_log_detail(log_id: str, current_user: dict = Depends(get_current_user)):
    """获取操作日志详情"""
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取操作日志详情失败: {e!s}")


@router.post("/clear", response_model=ClearLogsResponse)
async def clear_operation_logs(request: ClearLogsRequest, current_user: dict = Depends(get_current_user)):
    """清空操作日志"""
    try:
        logger.info(f"🗑️ 用户 {current_user['username']} 清空操作日志")

        service = get_operation_log_service()
        result = await service.clear_logs(days=request.days, action_type=request.action_type)
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"清空操作日志失败: {e!s}")


@router.post("/create")
async def create_operation_log(
    log_data: OperationLogCreate, request: Request, current_user: dict = Depends(get_current_user),
):
    """手动创建操作日志"""
    try:
        ...
        return {"success": True, "data": {"log_id": log_id}, "message": "创建操作日志成功"}

    except Exception as e:
        logger.error(f"创建操作日志失败: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"创建操作日志失败: {e!s}")
...
async def export_logs_csv(
    ...
    try:
        ...
        query = OperationLogQuery(
            page=1,
            page_size=100000,  # Phase 5d-1: 改为 100000，超出 Pydantic le=100 限制时直接绕过验证
            start_date=start_date,
            end_date=end_date,
            action_type=action_type,
        )
        ...
        # 写入表头
        writer.writerow(["时间", "用户", "操作类型", "操作内容", "状态", "耗时(ms)", "IP地址", "错误信息"])
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"导出操作日志CSV失败: {e!s}")
