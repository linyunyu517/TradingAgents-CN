...
async def get_cache_stats(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取缓存统计失败: {e!s}")


@router.delete("/cleanup")
async def cleanup_old_cache(
    days: int = Query(7, ge=1, le=30, description="清理多少天前的缓存"), current_user: dict = Depends(get_current_user),
):
    """
    清理过期缓存
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"清理缓存失败: {e!s}")
...
async def get_cache_details(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取缓存详情失败: {e!s}")
...
async def get_cache_backend_info(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            backend_info = {"system": "file", "primary_backend": "file", "fallback_enabled": False}
        ...
        raise HTTPException(status_code=500, detail=f"获取缓存后端信息失败: {e!s}")
