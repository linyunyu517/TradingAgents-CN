...
from app.services.progress.tracker import get_progress_file_path
...
async def task_progress_generator(task_id: str, user_id: str):
    ...
    r = get_redis_client()
    if r is None:
        logger.warning(f"⚠️ [方案C] Redis不可用，SSE降级为文件轮询: task={task_id}")
        # 方案C：文件轮询降级
        yield f'event: info\ndata: {{"task_id": "{task_id}", "message": "Redis不可用，使用文件进度轮询"}}\n\n'
        try:
            poll_interval = getattr(settings, "PROGRESS_POLL_INTERVAL", 2.0)
            max_idle = getattr(settings, "SSE_TASK_MAX_IDLE_SECONDS", 300)
                ...
                if os.path.exists(progress_file):
                    try:
                        ...
                        if mtime > last_mtime:
                            ...
                            yield f'event: progress\ndata: {json.dumps({"task_id": task_id, **data})}\n\n'
                            if status in ("completed", "error", "cancelled"):
                                logger.info(f"📡 [方案C] 文件进度已完成: task={task_id}, status={status}")
                                return
                            idle_since = time.time()  # 有更新，重置空闲计时
                    except (OSError, json.JSONDecodeError) as e:
                        logger.debug(f"📡 [方案C] 读取进度文件异常: {e}")
                else:
                    # 文件还不存在，等待
                    idle_elapsed = time.time() - idle_since
                    if idle_elapsed > max_idle:
                        yield f'event: error\ndata: {{"task_id": "{task_id}", "message": "等待超时，文件未生成"}}\n\n'
                        return

                await asyncio.sleep(poll_interval)
        except asyncio.CancelledError:
            logger.info(f"📡 [方案C] SSE连接断开: task={task_id}")
            return
    pubsub = None
