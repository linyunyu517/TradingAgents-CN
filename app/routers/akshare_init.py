...
async def test_akshare_connection():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"连接测试失败: {e!s}")
...
async def start_full_initialization(
    ...
    try:
        ...
        background_tasks.add_task(
            _run_full_initialization_background, request.historical_days, not request.skip_if_exists,
        )
        ...
        return {
            ...
            "data": {
                ...
                "parameters": {"historical_days": request.historical_days, "force": not request.skip_if_exists},
            },
        ...
        raise HTTPException(status_code=500, detail=f"启动初始化失败: {e!s}")


@router.post("/start-basic-sync")
async def start_basic_sync(
    request: SyncRequest, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user),
):
    """
    启动基础信息同步
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"启动同步失败: {e!s}")
...
async def stop_initialization(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"停止任务失败: {e!s}")
...
async def _run_full_initialization_background(historical_days: int, force: bool):
    ...
    try:
        ...
        result = await service.run_full_initialization(historical_days=historical_days, skip_if_exists=not force)
        ...
        _initialization_status.update({"is_running": False, "result": {"success": False, "error": str(e)}})
