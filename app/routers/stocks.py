...
async def get_quote(
    ...
    if market in ["HK", "US"]:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取行情失败: {e!s}")
...
async def get_fundamentals(
    ...
    if market in ["HK", "US"]:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取基础信息失败: {e!s}")
    ...
    total_mv = realtime_market_cap or b.get("total_mv")
...
async def get_kline(
    ...
    if market in ["HK", "US"]:
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取K线数据失败: {e!s}")
    ...
    if not items:
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"获取K线数据失败: {e!s}")
