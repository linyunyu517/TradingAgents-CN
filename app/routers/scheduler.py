...
async def verify_scheduler_service(
    service: SchedulerService | None = Depends(get_scheduler_service),
) -> SchedulerService:
    """
    验证调度器服务是否可用。
    当 get_scheduler_service() 返回 None 时，直接返回 503 Service Unavailable，
    避免 AttributeError 导致 500。
    """
    if service is None:
        raise HTTPException(
            status_code=503,
            detail="调度器服务不可用（调度器未初始化）",
        )
    return service
...
@router.get("/jobs")
async def list_jobs(
    user: dict = Depends(get_current_user), service: SchedulerService = Depends(verify_scheduler_service),
):
    """
    获取所有定时任务列表
...
async def update_job_metadata_route(
    ...
    service: SchedulerService = Depends(verify_scheduler_service),
):
    """
    更新任务元数据（触发器名称和备注）
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"更新任务元数据失败: {e!s}")


@router.get("/jobs/{job_id}")
async def get_job_detail(
    job_id: str, user: dict = Depends(get_current_user), service: SchedulerService = Depends(verify_scheduler_service),
):
    """
    获取任务详情
...
async def pause_job(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"暂停任务失败: {e!s}")
...
async def resume_job(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"恢复任务失败: {e!s}")
...
async def trigger_job(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"触发任务失败: {e!s}")
...
async def get_job_history(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取执行历史失败: {e!s}")
...
async def cancel_execution(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"取消任务失败: {e!s}")
...
async def mark_execution_failed(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"标记失败: {e!s}")
...
async def delete_execution(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"删除执行记录失败: {e!s}")
