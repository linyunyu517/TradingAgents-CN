...
# 全局初始化状态跟踪
_initialization_status = {"is_running": False, "current_step": None, "progress": None, "started_at": None, "task": None}
...
async def get_database_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取数据库状态失败: {e!s}")
...
async def get_initialization_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取初始化状态失败: {e!s}")


@router.post("/start-basic", response_model=dict)
async def start_basic_initialization(background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user)):
    """
    启动基础信息初始化
    仅同步股票基础信息，适合快速初始化
    """
    ...
    try:
        ...
        return ok(data={"message": "基础信息初始化已启动"}, message="基础信息初始化任务已在后台启动")

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"启动基础信息初始化失败: {e!s}")


@router.post("/start-full", response_model=dict)
async def start_full_initialization(
    request: InitializationRequest, background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_user),
):
    """
    启动完整数据初始化
    ...
    try:
        ...
        background_tasks.add_task(
            _run_full_initialization, request.historical_days, not request.skip_if_exists or request.force_update,
        )
        ...
        raise HTTPException(status_code=500, detail=f"启动完整数据初始化失败: {e!s}")
...
async def stop_initialization(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        _initialization_status.update(
            {"is_running": False, "current_step": None, "progress": None, "started_at": None, "task": None},
        )

        return ok(data={"message": "初始化任务已停止"}, message="初始化任务停止成功")

    except Exception as e:
        raise HTTPException(status_code=500, detail=f"停止初始化任务失败: {e!s}")


async def _run_basic_initialization():
    """运行基础信息初始化（后台任务）"""
    _initialization_status.update(
        {"is_running": True, "current_step": "基础信息初始化", "progress": "0/1", "started_at": datetime.utcnow()},
    )
    ...
    try:
        ...
        _initialization_status.update({"is_running": False, "current_step": "完成", "progress": "1/1"})

    except Exception as e:
        _initialization_status.update({"is_running": False, "current_step": f"失败: {e!s}", "progress": "错误"})


async def _run_full_initialization(historical_days: int, force_update: bool):
    """运行完整数据初始化（后台任务）"""
    _initialization_status.update(
        {"is_running": True, "current_step": "准备初始化", "progress": "0/6", "started_at": datetime.utcnow()},
    )
    ...
    try:
        ...
        # 运行初始化
        result = await service.run_full_initialization(historical_days=historical_days, skip_if_exists=not force_update)
        ...
        _initialization_status.update(
            {"is_running": False, "current_step": f"失败: {e!s}", "progress": "错误", "task": None},
        )
