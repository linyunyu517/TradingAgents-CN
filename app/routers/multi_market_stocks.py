...
async def search_stocks(
    ...
    if market not in ["CN", "HK", "US"]:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"不支持的市场类型: {market}")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"搜索失败: {e!s}")
...
async def get_stock_info(
    ...
    try:
        ...
        if not stock_info:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"未找到股票: {market}:{code}")
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票信息失败: {e!s}")


@router.get("/{market}/stocks/{code}/quote", response_model=dict)
async def get_stock_quote(market: str, code: str, current_user: dict = Depends(get_current_user)):
    """
    获取股票实时行情（支持多市场）
    ...
    try:
        ...
        if not quote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"未找到股票行情: {market}:{code}")
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取股票行情失败: {e!s}")
...
async def get_stock_daily_quotes(
    ...
    try:
        quotes = await service.get_daily_quotes(market, code, start_date, end_date, limit)

        return ok(data={"code": code, "market": market, "quotes": quotes, "total": len(quotes)})
    except Exception as e:
        logger.error(f"❌ 获取历史K线失败: market={market}, code={code}, error={e}", exc_info=True)
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"获取历史K线失败: {e!s}")
