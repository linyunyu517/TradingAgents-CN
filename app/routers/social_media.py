...
async def save_social_media_messages(request: SocialMediaBatchRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"保存社媒消息失败: {e!s}")
...
async def query_social_media_messages(request: SocialMediaQueryRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"查询社媒消息失败: {e!s}")
...
async def get_latest_messages(
    ...
    try:
        ...
        return ok(
            data={"messages": messages, "count": len(messages), "symbol": symbol, "platform": platform},
...
async def search_messages(
    ...
    try:
        ...
        return ok(
            data={"messages": messages, "count": len(messages), "query": query, "symbol": symbol, "platform": platform},
...
async def get_supported_platforms():
    ...
    return ok(data={"platforms": platforms, "count": len(platforms)}, message="支持的平台列表获取成功")
...
async def get_sentiment_analysis(
    ...
    try:
        ...
        # 查询消息
        params = SocialMediaQueryParams(
            symbol=symbol, platform=platform, start_time=start_time, end_time=end_time, limit=1000,
        )
        ...
        raise HTTPException(status_code=500, detail=f"情绪分析失败: {e!s}")
...
async def health_check():
    ...
    try:
        ...
        return ok(
            data={"status": "healthy", "total_messages": count, "service": "social_media_service"},
            message="社媒消息服务运行正常",
        )
