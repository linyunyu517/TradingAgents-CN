...
class FavoritesService:
            ...
            try:
                ...
                if missing:
                    try:
                        ...
                        logger.warning("获取在线行情失败，使用缓存数据", exc_info=True)
            except Exception:
        ...
        try:
            ...
            if is_oid:
                ...
                result = await db.users.update_one(
                    ...
                    {"$push": {"favorite_stocks": favorite_stock}, "$setOnInsert": {"favorite_stocks": []}},
                )
                logger.info(
            ...
            logger.error(f"❌ [add_favorite] 添加自选股异常: {type(e).__name__}: {e!s}", exc_info=True)
            raise
    ...
    async def remove_favorite(self, user_id: str, stock_code: str) -> bool:
        ...
        if self._is_valid_object_id(user_id):
            # 先尝试使用 ObjectId 查询
            result = await db.users.update_one(
                {"_id": ObjectId(user_id)}, {"$pull": {"favorite_stocks": {"stock_code": stock_code}}},
            )
            # 如果 ObjectId 查询失败，尝试使用字符串查询
            if result.matched_count == 0:
                result = await db.users.update_one(
                    {"_id": user_id}, {"$pull": {"favorite_stocks": {"stock_code": stock_code}}},
                )
            return result.modified_count > 0
        result = await db.user_favorites.update_one(
            {"user_id": user_id},
            {"$pull": {"favorites": {"stock_code": stock_code}}, "$set": {"updated_at": datetime.utcnow()}},
        )
        return result.modified_count > 0
    ...
    async def is_favorite(self, user_id: str, stock_code: str) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ [is_favorite] 检查自选股异常: {type(e).__name__}: {e!s}", exc_info=True)
            raise
