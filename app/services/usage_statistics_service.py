...
class UsageStatisticsService:
    ...
    async def get_usage_statistics(
        self, days: int = 7, provider: str | None = None, model_name: str | None = None,
    ) -> UsageStatistics:
        """获取使用统计"""
        try:
            db = get_mongo_db()
            ...
            stats.by_provider = {
                k: {**v, "cost_by_currency": dict(v["cost_by_currency"])} for k, v in by_provider.items()
            }
    ...
    async def delete_old_records(self, days: int = 90) -> int:
        ...
        try:
            ...
            # 删除旧记录
            result = await collection.delete_many({"timestamp": {"$lt": cutoff_date.isoformat()}})
