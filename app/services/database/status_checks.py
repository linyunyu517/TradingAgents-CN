...
async def get_mongodb_status() -> dict[str, Any]:
    try:
        ...
        # 🔧 FIX: serverStatus may return binary data (MongoDB 8.0+) that
        #     triggers 'utf-8' codec can't decode byte 0xc1 in pymongo/motor.
        #     Wrap it separately so a serialization glitch does not cause the
        #     entire status check to report disconnected=False.
        try:
            server_status = await db.command("serverStatus")
        except Exception:
            server_status = {"uptime": 0, "connections": {}, "mem": {}}
        return {
            "connected": True,
