...
class DataConsistencyChecker:
    ...
    def check_daily_basic_consistency(
        self, primary_data: pd.DataFrame, secondary_data: pd.DataFrame, primary_source: str, secondary_source: str,
        ...
        try:
            ...
            for metric in ["pe", "pb", "total_mv"]:
                comparison = self._compare_metric(primary_data, secondary_data, common_stocks, metric)
                if comparison:
    ...
    def _compare_metric(
        self, df1: pd.DataFrame, df2: pd.DataFrame, common_stocks: list[str], metric: str,
    ...
    def _calculate_overall_consistency(
        self, comparisons: list[FinancialMetricComparison], primary_source: str, secondary_source: str,
    ) -> DataConsistencyResult:
        """计算整体一致性结果"""
        if not comparisons:
            return DataConsistencyResult(
    ...
    def resolve_data_conflicts(
        self, primary_data: pd.DataFrame, secondary_data: pd.DataFrame, consistency_result: DataConsistencyResult,
        ...
        return (
            primary_data,
            f"数据源存在严重不一致（置信度: {consistency_result.confidence_score:.2f}），建议检查数据源",
        )
