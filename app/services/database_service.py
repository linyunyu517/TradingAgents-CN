...
class DatabaseService:
    ...
    async def get_database_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            # 并行获取所有集合的统计
            collections_info = await asyncio.gather(*[get_collection_stats(name) for name in collection_names])
            ...
            raise Exception(f"获取数据库统计失败: {e!s}")
    ...
    async def create_backup(
        self, name: str, collections: list[str] | None = None, user_id: str | None = None,
    ) -> dict[str, Any]:
        """
        创建数据库备份（自动选择最佳方法）
        ...
        if _db_backups._check_mongodump_available():
            ...
            return await _db_backups.create_backup_native(
                name=name, backup_dir=self.backup_dir, collections=collections, user_id=user_id,
            )
    ...
    async def export_data(
        self, collections: list[str] | None = None, format: str = "json", sanitize: bool = False,
    ) -> str:
        """导出数据（委托子模块）"""
