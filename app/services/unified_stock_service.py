...
class UnifiedStockService:
    ...
    async def get_stock_info(self, market: str, code: str, source: str | None = None) -> dict | None:
        """
        获取股票基础信息（支持多数据源）
    ...
    async def _get_source_priority(self, market: str) -> list[str]:
        ...
        try:
            ...
            groupings = (
                await self.db.datasource_groupings.find({"market_category_id": market_category_id, "enabled": True})
                .sort("priority", -1)
                .to_list(length=None)
            )
    ...
    async def get_daily_quotes(
        self, market: str, code: str, start_date: str | None = None, end_date: str | None = None, limit: int = 100,
    ) -> list[dict]:
        """
        获取历史K线数据
    ...
    async def get_supported_markets(self) -> list[dict]:
        ...
        """
        return [
            {"code": "CN", "name": "A股", "name_en": "China A-Share", "currency": "CNY", "timezone": "Asia/Shanghai"},
            {
                "code": "HK",
                "name": "港股",
            ...
            {"code": "US", "name": "美股", "name_en": "US Stock", "currency": "USD", "timezone": "America/New_York"},
        ]
