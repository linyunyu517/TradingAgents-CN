...
class ModelCapabilityService:
    ...
    def _get_model_capability_with_mapping(self, model_name: str) -> tuple[int, str | None]:
        ...
        # 2. 尝试解析聚合渠道模型名
        _provider, original_model = self._parse_aggregator_model_name(model_name)
    ...
    def validate_model_pair(self, quick_model: str, deep_model: str, research_depth: str) -> dict[str, Any]:
        ...
        if ModelFeature.TOOL_CALLING not in quick_features:
            ...
            # 🔥 [Bug L 修复] 推荐支持工具调用的替代模型
            # 查找替代模型：支持 TOOL_CALLING 特性的快速模型
            alternative_models = []
            for model_name, config in DEFAULT_MODEL_CAPABILITIES.items():
                features = config.get("features", [])
                roles = config.get("suitable_roles", [])
                if ModelFeature.TOOL_CALLING in features:
                    if ModelRole.QUICK_ANALYSIS in roles or ModelRole.BOTH in roles:
                        alternative_models.append(model_name)
            if alternative_models:
                alt_list = "、".join(alternative_models[:5])
                result["recommendations"].append(f"💡 建议将快速模型替换为支持工具调用的模型：{alt_list}")
