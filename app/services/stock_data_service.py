...
class StockDataService:
    ...
    async def get_stock_basic_info(self, symbol: str, source: str | None = None) -> StockBasicInfoExtended | None:
        """
        获取股票基础信息
        Args:
            symbol: 6位股票代码
            source: 数据源 (tushare/multi_source)，默认优先级：tushare > multi_source
        Returns:
        ...
        try:
            ...
            if source:
                ...
                # 🔥 未指定数据源，按优先级查询
                source_priority = ["tushare", "multi_source"]
                doc = None
        ...
        try:
            ...
            if not source:
                ...
                enabled_sources = [
                    ...
                    if ds.enabled and ds.type.lower() in ["tushare"]
                ]

                if not enabled_sources:
                    enabled_sources = ["tushare"]
            ...
            cursor = db[self.basic_info_collection].find(query, {"_id": 0}).skip(skip).limit(page_size)
    ...
    async def update_stock_basic_info(self, symbol: str, update_data: dict[str, Any], source: str = "tushare") -> bool:
        """
        更新股票基础信息
        Args:
            symbol: 6位股票代码
            update_data: 更新数据
            source: 数据源 (tushare)，默认 tushare
        Returns:
            bool: 更新是否成功
        """
        try:
    ...
    async def update_market_quotes(self, symbol: str, quote_data: dict[str, Any]) -> bool:
        """
        更新实时行情数据
        Args:
            symbol: 6位股票代码
