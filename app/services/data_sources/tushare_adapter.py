...
class TushareAdapter(DataSourceAdapter):
        ...
        try:
            ...
            for _, row in df.iterrows():  # type: ignore
                ...
                if not ts_code or "." not in ts_code:
                    continue
                code6 = ts_code.split(".", maxsplit=1)[0].zfill(6)
                close = (
                ...
                pre_close = (
                    ...
                    if hasattr(self, "_safe_float")
                    else (float(row.get("pre_close")) if row.get("pre_close") is not None else None)
                )
                amount = (
                    self._safe_float(row.get("amount"))
                    if hasattr(self, "_safe_float")
                    else (float(row.get("amount")) if row.get("amount") is not None else None)
                )
    ...
    def get_news(self, code: str, days: int = 2, limit: int = 50, include_announcements: bool = True):
        ...
        try:
            ...
            logger.debug("Tushare 公告获取失败: %s", ts_code)
        # Attempt news
        try:
            if hasattr(api, "news"):
            ...
            logger.debug("Tushare 新闻获取失败: %s", ts_code)
    ...
    def find_latest_trade_date(self) -> str | None:
        ...
        try:
            today = datetime.now()
            for delta in range(10):  # up to 10 days back
