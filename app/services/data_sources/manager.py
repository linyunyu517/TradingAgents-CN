...
class DataSourceManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("📊 Tushare Pro 数据源已加载")
    ...
    def _load_priority_from_database(self):
        ...
        try:
            ...
            # 查询 A股市场的数据源分组配置
            groupings = list(groupings_collection.find({"market_category_id": "a_shares", "enabled": True}))
    ...
    def get_stock_list_with_fallback(
        self, preferred_sources: list[str] | None = None,
    ) -> tuple[pd.DataFrame | None, str | None]:
        """
        获取股票列表，支持指定优先数据源

        Args:
            preferred_sources: 优先使用的数据源列表，例如 ['tushare']
                             如果为 None，则按照默认优先级顺序
    ...
    def get_daily_basic_with_fallback(
        self, trade_date: str, preferred_sources: list[str] | None = None,
    ) -> tuple[pd.DataFrame | None, str | None]:
        """
        获取每日基础数据，支持指定优先数据源
    ...
    def get_kline_with_fallback(
        self, code: str, period: str = "day", limit: int = 120, adj: str | None = None,
