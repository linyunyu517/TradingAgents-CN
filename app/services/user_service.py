...
安全加固：
- [Bug B02] 移除硬编码密码 admin123，改为从环境变量 ADMIN_DEFAULT_PASSWORD 读取
- [Bug B02] 当未提供密码且环境变量未设置时，生成随机密码并记录到日志
"""
...
class UserService:
    ...
    def __init__(self):
        self.client = MongoClient(
            settings.MONGO_URI,
            serverSelectionTimeoutMS=getattr(settings, "MONGO_SERVER_SELECTION_TIMEOUT_MS", 5000),
            connectTimeoutMS=getattr(settings, "MONGO_CONNECT_TIMEOUT_MS", 30000),
            socketTimeoutMS=getattr(settings, "MONGO_SOCKET_TIMEOUT_MS", 60000),
        )
    ...
    def close(self):
        ...
        try:
            ...
            logger.debug("关闭 MongoDB 连接失败", exc_info=True)

    def __del__(self):
        """析构函数，确保连接被关闭（带守卫，防止 AttributeError）"""
    ...
    async def create_admin_user(
        self, username: str = "admin", password: str | None = None, email: str = "admin@tradingagents.cn",
    ) -> User | None:
        """
        创建管理员用户

        🔒 [Bug B02] 安全加固：
        - password 参数默认为 None，不再使用硬编码 "admin123"
        - 优先使用参数传入的密码
        - 其次从环境变量 ADMIN_DEFAULT_PASSWORD 读取
        - 如果都未设置，生成随机密码并记录到日志

        Args:
            username: 用户名
            password: 密码（可选，默认为 None）
            email: 邮箱
        ...
        try:
            ...
            if not resolved_password:
                # 尝试从环境变量读取
                resolved_password = os.environ.get("ADMIN_DEFAULT_PASSWORD")

            if not resolved_password:
                # 🔒 [Bug B02] 生成随机密码，不硬编码回退
                alphabet = string.ascii_letters + string.digits + "!@#$%^&*"
                resolved_password = "".join(secrets.choice(alphabet) for _ in range(16))
                logger.warning("=" * 60)
                logger.warning("⚠️ [安全注意] ADMIN_DEFAULT_PASSWORD 环境变量未设置")
                logger.warning("   已为管理员生成随机密码，请立即保存并记录！")
                logger.warning(f"   🔑 生成的随机密码: {resolved_password}")
                logger.warning("   💡 请设置 ADMIN_DEFAULT_PASSWORD 环境变量以避免每次随机生成")
                logger.warning("=" * 60)
            ...
            admin_doc = {
                "username": username,
                "email": email,
                "hashed_password": self.hash_password(resolved_password),
                "is_active": True,
                "is_verified": True,
                "is_admin": True,
                "created_at": datetime.utcnow(),
