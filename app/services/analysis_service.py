...
class AnalysisService:
    ...
    def __init__(self):
        # 获取Redis客户端（Redis不可用时降级，不影响核心分析功能）
        redis_client = get_redis_client()
        if redis_client is not None:
            self.queue_service = QueueService(redis_client)
        else:
            logger.warning("⚠️ [BUG-009] Redis不可用，AnalysisService队列功能降级")
    ...
    def _get_trading_graph(self, config: dict[str, Any]) -> TradingAgentsGraph:
        ...
        if config_key not in self._trading_graph_cache:
            # 🐛 [BUG-037 扩散修复] 配置键名桥接：与 simple_analysis_service 保持一致
            # create_analysis_config 使用 deep_analysis_model/quick_analysis_model 等键名，
            # 但 TradingAgentsGraph 读取 deep_think_llm/quick_think_llm。
            # 不桥接会导致 DEFAULT_CONFIG 中的 OpenAI 默认模型名被使用。
            _bridged = dict(config)
            _CONFIG_KEY_BRIDGE = {
            ...
            for _src_key, _dst_key in _CONFIG_KEY_BRIDGE.items():
                if config.get(_src_key):
                    _bridged[_dst_key] = config[_src_key]
    ...
    def _execute_analysis_sync_with_progress(
        ...
        try:
            ...
            quick_model = (
                getattr(task.parameters, "quick_analysis_model", None) or unified_config.get_quick_analysis_model()
            )
            deep_model = (
                getattr(task.parameters, "deep_analysis_model", None) or unified_config.get_deep_analysis_model()
            )
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                result = await loop.run_in_executor(
                    executor, self._execute_analysis_sync_with_progress, task, progress_tracker,
                )
    ...
    async def submit_single_analysis(self, user_id: str, request: SingleAnalysisRequest) -> dict[str, Any]:
        ...
        try:
            ...
            if not getattr(params, "quick_analysis_model", None):
                params.quick_analysis_model = effective_settings.get("quick_analysis_model", "deepseek-v4-flash")
            if not getattr(params, "deep_analysis_model", None):
                params.deep_analysis_model = effective_settings.get("deep_analysis_model", "deepseek-v4-pro")
            ...
            # 🐛 [BUG-028 Fix A 扩散] 注册到 MemoryStateManager
            try:
                ...
                memory_manager.create_task_sync(
                    ...
                    parameters=params.model_dump() if hasattr(params, 'model_dump') else (params.dict() if hasattr(params, 'dict') else params),
                )
    ...
    async def submit_batch_analysis(self, user_id: str, request: BatchAnalysisRequest) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                logger.warning("分析服务调用失败", exc_info=True)
            ...
            for task in tasks:
                ...
                await self.queue_service.enqueue_task(
                    user_id=str(converted_user_id), symbol=task.symbol, params=queue_params, batch_id=task.batch_id,
                )
    ...
    async def execute_analysis_task(
        self, task: AnalysisTask, progress_callback: Callable[[int, str], None] | None = None,
    ) -> AnalysisResult:
        """执行单个分析任务"""
        try:
            logger.info(f"开始执行分析任务: {task.task_id} - {task.symbol}")
    ...
    async def _record_token_usage(self, task: AnalysisTask, result: AnalysisResult, provider: str, model_name: str):
        """记录 token 使用情况"""
        try:
            # 从结果中提取 token 使用信息
