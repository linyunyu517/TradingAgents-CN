...
class SocialMediaService:
    ...
    async def save_social_media_messages(self, messages: list[dict[str, Any]]) -> dict[str, int]:
        """
        批量保存社媒消息
        ...
        try:
            ...
            for message in messages:
                ...
                filter_dict = {"message_id": message.get("message_id"), "platform": message.get("platform")}
    ...
    async def query_social_media_messages(self, params: SocialMediaQueryParams) -> list[dict[str, Any]]:
        """
        查询社媒消息
    ...
    async def get_latest_messages(
        self, symbol: str | None = None, platform: str | None = None, limit: int = 20,
    ) -> list[dict[str, Any]]:
        """获取最新社媒消息"""
        params = SocialMediaQueryParams(
            symbol=symbol, platform=platform, limit=limit, sort_by="publish_time", sort_order=-1,
        )
        return await self.query_social_media_messages(params)

    async def search_messages(
        self, query: str, symbol: str | None = None, platform: str | None = None, limit: int = 50,
    ) -> list[dict[str, Any]]:
        """全文搜索社媒消息"""
        try:
            collection = await self._get_collection()
            ...
            # 执行搜索
            cursor = collection.find(search_query, {"score": {"$meta": "textScore"}}).sort(
