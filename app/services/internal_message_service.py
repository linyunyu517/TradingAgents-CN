...
class InternalMessageService:
    ...
    async def save_internal_messages(self, messages: list[dict[str, Any]]) -> dict[str, int]:
        """
        批量保存内部消息
    ...
    async def query_internal_messages(self, params: InternalMessageQueryParams) -> list[dict[str, Any]]:
        """
        查询内部消息
    ...
    async def search_messages(
        self, query: str, symbol: str | None = None, access_level: str | None = None, limit: int = 50,
    ) -> list[dict[str, Any]]:
        """全文搜索内部消息"""
        try:
            collection = await self._get_collection()
    ...
    async def get_research_reports(
        self, symbol: str | None = None, department: str | None = None, limit: int = 20,
    ) -> list[dict[str, Any]]:
        """获取研究报告"""
        params = InternalMessageQueryParams(
            symbol=symbol,
    ...
    async def get_analyst_notes(
        self, symbol: str | None = None, author: str | None = None, limit: int = 20,
    ) -> list[dict[str, Any]]:
        """获取分析师笔记"""
        params = InternalMessageQueryParams(
            symbol=symbol,
