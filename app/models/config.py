...
class LLMProvider(BaseModel):
    ...
    # ── 自动解密：从 MongoDB 读出后立即解密敏感字段 ──────────────────
    @model_validator(mode="before")
    @classmethod
    def _decrypt_sensitive_fields(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if data.get("api_key"):
                data["api_key"] = decrypt_field(data["api_key"])
            if data.get("api_secret"):
                data["api_secret"] = decrypt_field(data["api_secret"])
        return data

    # ── 自动加密：序列化/保存时加密敏感字段 ──────────────────────────
    @field_serializer("api_key", "api_secret", when_used="json-unless-none")
    def _encrypt_sensitive(self, value: str | None) -> str | None:
...
class LLMConfig(BaseModel):
    ...
    capability_level: int = Field(
        default=2, ge=1, le=5, description="模型能力等级(1-5): 1=基础, 2=标准, 3=高级, 4=专业, 5=旗舰",
    )
    ...
    recommended_depths: list[str] = Field(
        default_factory=lambda: ["快速", "基础", "标准"], description="推荐的分析深度级别",
    )
    thinking: dict[str, Any] | None = Field(None, description='思考模式配置 (如: {"type": "enabled"})')
    performance_metrics: dict[str, Any] | None = Field(
        default=None, description="性能指标: speed(速度1-5), cost(成本1-5), quality(质量1-5)",
    )
