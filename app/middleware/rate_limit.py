...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def check_rate_limit(self, user_id: str, endpoint: str):
        ...
        # 构建Redis键
        rate_key = RedisKeys.USER_RATE_LIMIT.format(user_id=user_id, endpoint=endpoint.replace("/", "_"))
        ...
        if current_count > rate_limit:
            logger.warning(
                f"速率限制触发 - 用户: {user_id}, 端点: {endpoint}, 当前计数: {current_count}, 限制: {rate_limit}",
            )
        ...
        logger.debug(f"速率限制检查通过 - 用户: {user_id}, 端点: {endpoint}, 当前计数: {current_count}/{rate_limit}")
...
class QuotaMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, daily_quota: int = 1000):
        ...
        # 需要计入配额的端点
        self.quota_endpoints = {"/api/analysis/single", "/api/analysis/batch", "/api/screening/filter"}
    ...
    async def check_daily_quota(self, user_id: str):
        ...
        if current_usage > self.daily_quota:
            logger.warning(f"每日配额超限 - 用户: {user_id}, 今日使用: {current_usage}, 配额: {self.daily_quota}")
        ...
        logger.debug(f"配额检查通过 - 用户: {user_id}, 今日使用: {current_usage}/{self.daily_quota}")
