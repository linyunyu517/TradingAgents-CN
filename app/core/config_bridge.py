...
def bridge_config_to_env():
    ...
    try:
        ...
        use_mongodb_storage = os.getenv("USE_MONGODB_STORAGE", "false")
        os.environ["USE_MONGODB_STORAGE"] = use_mongodb_storage
        logger.info(f"  ✓ 桥接 USE_MONGODB_STORAGE: {use_mongodb_storage}")
        bridged_count += 1

        # 桥接 MongoDB 连接字符串
        # 优先级: 环境变量 > Pydantic Settings MONGO_URI（含默认值 localhost:27017）
        from app.core.config import settings

        mongodb_conn_str = os.getenv("MONGODB_CONNECTION_STRING")
        if not mongodb_conn_str or mongodb_conn_str.strip() == "":
            mongodb_conn_str = settings.MONGO_URI
            os.environ["MONGODB_CONNECTION_STRING"] = mongodb_conn_str
            logger.info(f"  ✓ 使用 Pydantic Settings 构建 MONGODB_CONNECTION_STRING ({mongodb_conn_str})")
            bridged_count += 1
        else:
            os.environ["MONGODB_CONNECTION_STRING"] = mongodb_conn_str
        ...
        try:
            ...
            if mongodb_conn:
                ...
                logger.info("  📋 MONGODB_CONNECTION_STRING: <未设置，使用 JSON 文件存储>")
            ...
            if use_mongodb.lower() == "true":
                if not mongodb_conn:
                    logger.warning("⚠️ MONGODB_CONNECTION_STRING 未设置，跳过 MongoDB 初始化，使用 JSON 文件存储")
                    config_manager.mongodb_storage = None
                else:
                    try:
                        # 🔍 详细日志：显示完整的连接字符串（用于调试）
                        logger.info(f"  🔍 实际传入的连接字符串: {mongodb_conn[:30]}...")
        ...
        try:
            ...
            # 不在异步上下文中，使用 asyncio.run（带 30s 超时保护）
            asyncio.run(asyncio.wait_for(_sync_pricing_config_from_db(), timeout=30))
