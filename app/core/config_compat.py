...
class ConfigManagerCompat:
    ...
    def load_settings(self) -> dict[str, Any]:
        ...
        try:
            ...
            warnings.warn("加载系统设置失败，使用默认值", RuntimeWarning, stacklevel=2)
        return self._get_default_settings()
    ...
    def save_settings(self, settings_dict: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if loop.is_running():
                ...
                return False
            loop.run_until_complete(config_service.update_system_settings(settings_dict))
            return True
    ...
    def get_models(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            warnings.warn("加载模型列表失败，返回空列表", RuntimeWarning, stacklevel=2)
        return []
...
class TokenTrackerCompat:
    ...
    def track_usage(self, provider: str, model_name: str, input_tokens: int, output_tokens: int, cost: float = 0.0):
        """
        记录 Token 使用量
