...
class DatabaseManager:
    ...
    async def init_mongodb(self):
        ...
        try:
            ...
            # 创建MongoDB客户端，配置连接池
            # CYCLE2-003: 添加 w="majority" writeConcern，确保重要数据写入持久化
            self.mongo_client = AsyncIOMotorClient(
                ...
                w="majority",  # CYCLE2-003: 写关注级别设置为 majority，保证主节点确认后返回，提升数据持久性
            )
    ...
    async def init_redis(self):
        ...
        try:
            ...
            await self._init_redis_no_auth()
    ...
    async def _init_redis_no_auth(self):
        ...
        try:
            ...
            logger.info("✅ Redis无密码连接成功建立（降级模式）")
            logger.info(f"🔗 连接池大小: {settings.REDIS_MAX_CONNECTIONS}")
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            health_status["mongodb"] = {"status": "unhealthy", "details": {"error": str(e)}}
            self._mongo_healthy = False
...
async def init_database():
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"⚠️ Redis连接初始化失败（不影响核心功能）: {redis_err}")
            redis_client = None
            redis_pool = None
...
def get_mongo_db_sync() -> Database:
    ...
    获取同步版本的MongoDB数据库实例
    用于非异步上下文（如普通函数调用）
    包含健康检查，在连接时执行 ping() 以提前诊断 MongoDB 不可用情况
    """
    ...
    if _sync_mongo_client is None:
        ...
        # 🆕 健康检查：连接后执行 ping()，提前诊断 MongoDB 不可用情况
        try:
            _sync_mongo_client.admin.command("ping")
            logger.info("✅ MongoDB同步连接健康检查通过")
        except Exception as e:
            logger.error(f"❌ MongoDB同步连接健康检查失败: {e}")
            _sync_mongo_client.close()
            _sync_mongo_client = None
            raise ConnectionError(f"MongoDB 连接失败: {e}")
...
def get_redis_client() -> Redis | None:
    """获取Redis客户端（Redis不可用时返回None，不抛异常）
