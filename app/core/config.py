...
class Settings(BaseSettings):
    ...
    # 🔒 [Bug B24] CORS 安全加固
    # - 禁止在生产环境使用 * 通配符（当 allow_credentials=True 时 FastAPI 会拒绝）
    # - 从环境变量 CORS_ORIGINS 读取，支持逗号分隔多个域名
    # - 默认仅允许本地开发地址，生产环境必须显式配置
    # 示例: CORS_ORIGINS=http://localhost:3000,https://example.com
    ALLOWED_ORIGINS: list[str] = Field(default_factory=lambda: ["http://localhost:3000"], alias="CORS_ORIGINS")
    ...
    # DeepSeek API 配置（从 .env 的 DEEPSEEK_API_KEY 读取）
    ...
    # 进度文件配置（方案C：文件式进度追踪）
    PROGRESS_DIR: str = Field(default="./data/progress", description="进度文件存储目录")
    PROGRESS_FILE_TTL_HOURS: int = Field(default=2, ge=1, le=168, description="进度文件保留时间（小时）")
    PROGRESS_POLL_INTERVAL: float = Field(default=2.0, ge=0.5, le=30, description="SSE 文件回退轮询间隔（秒）")
    ...
    TUSHARE_ENABLED: bool = Field(default=True, description="启用Tushare数据源（主数据源）")
    ...
    # BaoStock统一数据同步总开关（已切换为Tushare主数据源）
    BAOSTOCK_UNIFIED_ENABLED: bool = Field(default=False, description="启用BaoStock统一数据同步")

    # BaoStock数据同步任务配置
    BAOSTOCK_BASIC_INFO_SYNC_ENABLED: bool = Field(default=False, description="启用基础信息同步（已禁用）")
    ...
    BAOSTOCK_DAILY_QUOTES_SYNC_ENABLED: bool = Field(
        default=False, description="启用日K线同步（已禁用）",
    )
    BAOSTOCK_DAILY_QUOTES_SYNC_CRON: str = Field(
        default="0 16 * * 1-5", description="日K线同步CRON表达式",
    )  # 工作日收盘后16:00
    BAOSTOCK_HISTORICAL_SYNC_ENABLED: bool = Field(default=False, description="启用历史数据同步（已禁用）")
    BAOSTOCK_HISTORICAL_SYNC_CRON: str = Field(
        default="0 18 * * 1-5", description="历史数据同步CRON表达式",
    )  # 工作日18点
    BAOSTOCK_STATUS_CHECK_ENABLED: bool = Field(default=False, description="启用状态检查（已禁用）")
...
# RUNTIME-010: 使用 try/except 包装 Settings() 实例化，避免 .env 缺失时启动崩溃
# 如果 .env 文件不存在或格式错误，使用默认值继续启动
try:
    settings = Settings()
except Exception as _settings_err:
    import warnings as _warnings

    _warnings.warn(f".env 文件加载失败，使用默认配置: {_settings_err}", stacklevel=2)
...
def _read_major_version() -> str:
    ...
    if not v:
        try:
            v = Path(__file__).resolve().parents[2].joinpath("VERSION").read_text(encoding="utf-8").strip()
        except Exception:
            v = ""
...
def _default_instance_tag() -> str:
    user = os.getenv("TRADINGAGENTS_DB_USER", "").strip() or _get_username()
    host = (
...
def _get_username() -> str:
    """跨平台的用户名获取（兼容 Windows，确保路由到正确的数据库实例）"""
    # Windows 优先：直接使用 USERNAME 环境变量，避免 getpass.getuser() 因 pwd 模块崩溃
    username = os.getenv("USERNAME") or os.getenv("USER") or os.getenv("USERDOMAIN")
    if username:
        return username
    try:
        return getpass.getuser()
    except Exception:
        return "16660"  # 硬编码默认值，确保路由到 tradingagentscn_v0_16660
...
# RUNTIME-011: 将代理环境变量设置封装为函数，避免模块导入时立即修改 os.environ
# 这样在应用启动阶段某些库初始化后再调用，确保代理配置生效
def _apply_proxy_env():
    """将代理配置设置到环境变量，供 requests/httpx 等库读取"""
