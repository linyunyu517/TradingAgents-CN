...
async def publish_progress(task_id: str, message: str, step: int | None = None, total_steps: int | None = None):
    ...
    r = get_redis_client()
    if r is None:
        logger.warning(f"⚠️ [BUG-009] Redis不可用，跳过进度推送: {task_id}")
        return
    progress_data = {
        "task_id": task_id,
        "message": message,
...
async def process_task(task_id: str) -> None:
    r = get_redis_client()
    if r is None:
        logger.warning(f"⚠️ [BUG-009] Redis不可用，跳过任务处理: {task_id}")
        return
    ...
    try:
        ...
        try:
            ...
            result = {
                "symbol": symbol,
                "error": f"分析执行异常: {analysis_error!s}",
                "completed_at": datetime.now().isoformat(),
                "success": False,
            }
            status = "failed"
            await progress_callback(f"❌ 分析执行异常: {analysis_error!s}")
        ...
        await r.srem(SET_PROCESSING, task_id)
        await r.sadd(SET_FAILED, task_id)
        await publish_progress(task_id, f"❌ 处理失败: {e!s}")


async def worker_loop(stop_event: asyncio.Event):
    r = get_redis_client()
    if r is None:
        logger.error("❌ [BUG-009] Redis不可用，Worker无法启动")
        return
