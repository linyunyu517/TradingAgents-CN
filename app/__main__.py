...
# ============================================================================
# 🔥 [Bug #2 修复] 全局 UTF-8 编码设置 — 必须在任何 import 之前
# ============================================================================
# 问题: Windows 默认 GBK 编码无法输出 emoji（如 📊 ✅ ❌），导致 UnicodeEncodeError
# 修复: 在 import 任何模块之前设置 PYTHONIOENCODING=utf-8，覆盖控制台代码页
# ============================================================================
import os
import sys

# [R2 Fix M4] Docker 容器内跳过 PYTHONUTF8，避免 locale 不完整导致子进程静默崩溃
_IS_DOCKER = os.environ.get("DOCKER_CONTAINER") == "true" or os.path.exists("/.dockerenv")

if sys.platform == "win32":
    try:
        # 1. 设置环境变量，让 Python 全局使用 UTF-8（必须在所有 import 之前）
        ...
        try:
            ...
            pass  # 控制台代码页设置在启动早期，logger 尚未初始化
...
try:
    ...
    from app.main import _check_port_available
except Exception as e:
    import traceback
...
def main():
    ...
    env_vars_to_check = [
        ...
        (
            "JWT_SECRET",
            "***" if settings.JWT_SECRET != "change-me-in-production" else settings.JWT_SECRET,
    ...
    try:
        uvicorn.run("app.main:app", host=settings.HOST, port=settings.PORT, **uvicorn_config)
